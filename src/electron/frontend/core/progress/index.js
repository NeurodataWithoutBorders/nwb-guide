...
function encode(text) {
  ...
  return `${CRYPTO_VERSION}:${ENCRYPTION_IV.toString("hex")}:${Buffer.concat([
    encrypted,
    cipher.final(),
  ]).toString("hex")}`;
}
...
function decode(text) {
  ...
  if (text.slice(0, TEXT_CRYPTO_VERSION.length) !== CRYPTO_VERSION)
    return undefined;
  ...
  try {
    ...
    let decipher = crypto.createDecipheriv(
      CRYPTO_ALGORITHM,
      ENCRYPTION_KEY,
      Buffer.from(ENCRYPTION_IV_HEX, "hex"),
    );
  ...
  constructor() {
    ...
    if (exists) {
      const data = JSON.parse(
        fs ? fs.readFileSync(this.path) : localStorage.getItem(this.path),
      );
...
export const getEntries = () => {
  if (fs && !fs.existsSync(guidedProgressFilePath))
    fs.mkdirSync(guidedProgressFilePath, { recursive: true }); //Check if progress folder exists. If not, create it.
  const progressFiles = fs
    ? fs.readdirSync(guidedProgressFilePath)
...
const transformProgressFile = (progressFile) => {
  progressFile["page-before-exit"] = convertOldPath(
    progressFile["page-before-exit"],
  );
...
export const getAll = (progressFiles) => {
  return progressFiles.map((progressFile) => {
    ...
    return transformProgressFile(
      JSON.parse(
        fs
          ? fs.readFileSync(progressFilePath)
          : localStorage.getItem(progressFile),
      ),
    );
  });
};
...
export const get = (name) => {
  ...
  const exists = fs
    ? fs.existsSync(progressFilePath)
    : localStorage.getItem(progressFilePath) !== null;
  return transformProgressFile(
    exists
      ? JSON.parse(
          fs
            ? fs.readFileSync(progressFilePath)
            : localStorage.getItem(progressFilePath),
        )
      : {},
  );
};
...
export const deleteProgressCard = async (progressCardDeleteButton) => {
  ...
  const progressCardNameToDelete = progressCard
    .querySelector(".progress-file-name")
    .textContent.trim();
