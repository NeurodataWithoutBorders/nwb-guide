  ...
  if (previousDatasetName) {
    ...
    if (fs) fs.renameSync(oldProgressFilePath, newProgressFilePath);
      ...
      localStorage.setItem(
        newProgressFilePath,
        localStorage.getItem(oldProgressFilePath),
      );
...
export const updateAppProgress = (
  pageId,
  dataOrProjectName = {},
  projectName = typeof dataOrProjectName === "string"
    ? dataOrProjectName
    : undefined,
) => {
  ...
  // Is a project name
  if (dataOrProjectName === projectName)
    updateFile(
      dataOrProjectName,
      (data) => (data["page-before-exit"] = pageId),
    );
  // Is a data object
...
export const updateFile = (projectName, callback) => {
  ...
  if (fs) {
    if (!fs.existsSync(guidedProgressFilePath))
      fs.mkdirSync(guidedProgressFilePath, { recursive: true }); //create progress folder if one does not exist
