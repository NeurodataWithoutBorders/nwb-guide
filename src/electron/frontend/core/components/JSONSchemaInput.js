...
export function createTable(fullPath, { onUpdate, onThrow, overrides = {} }) {
  ...
  const relativePath = this.form?.base
    ? fullPath.slice(this.form.base.length)
    : fullPath;
  ...
  const commonValidationFunction = async (
    ...
    const returnedValue = errors.length
      ? errors
      : warnings.length
        ? warnings
        : result;
  ...
  const addPropertyKeyToSchema = (schema) => {
    ...
    schemaItemsRef.properties[tempPropertyKey] = {
      title: "Property Key",
      type: "string",
      pattern: name,
    };
  ...
  const createNestedTable = (
    id,
    value,
    { name: propName = id, nestedSchema = schema } = {},
  ) => {
    ...
    function setValueOnAccumulator(row, acc) {
      ...
      if (tempPropertyValueKey in row) {
        ...
        if (Array.isArray(propValue))
          acc[key] = propValue.reduce(
            (acc, row) => setValueOnAccumulator(row, acc),
            {},
          );
    ...
    const nestedIgnore = this.form?.ignore
      ? getIgnore(this.form?.ignore, schemaPath)
      : {};
    ...
    const tableMetadata = {
      ...
      onUpdate: function (path, newValue) {
        ...
        if (path.slice(-1)[0] === tempPropertyKey && keyAlreadyExists(newValue))
      ...
      validateOnChange: function (path, parent, newValue) {
        ...
        if (path.slice(-1)[0] === tempPropertyKey && resolvedKey !== newValue) {
          if (keyAlreadyExists(newValue)) {
            if (!previousValidValues[rowIdx])
              previousValidValues[rowIdx] = resolvedKey;
        ...
        const toIterate = updatedPath.filter(
        ...
        return commonValidationFunction(
          [],
          updatedPath,
          parent,
          newValue,
  ...
  if (isEditableObject(schema, this.value)) {
    ...
    const data = getEditableItems(this.value, this.pattern, {
      name,
      schema: schemaCopy,
  ...
  const tableMetadata = {
    ...
    validateOnChange: (...args) =>
      commonValidationFunction(relativePath, ...args),
...
export const isPatternProperties = (pattern) =>
  pattern && !isAdditionalProperties(pattern);
...
export const getEditableItems = (
  ...
  if (isPatternProperties(pattern)) {
    ...
    items = items.filter(([key]) => key in schema.properties);
...
const isFilesystemSelector = (name = "", format) => {
  if (Array.isArray(format))
    return format.map((f) => isFilesystemSelector(name, f)).every(Boolean)
      ? format
      : null;

  const matched = name.match(/(.+_)?(.+)_paths?/);
  if (!format && matched)
    format = matched[2] === "folder" ? "directory" : matched[2];
...
export class JSONSchemaInput extends LitElement {
  ...
  onUncaughtSchema = (schema) => {
    ...
    if (!isDevelopment) {
      if (this.form) {
        const inputContainer = this.form.shadowRoot.querySelector(
          `#${this.path.slice(-1)[0]}`,
        );
    ...
    if (schema.default)
      return `<pre>${JSON.stringify(schema.default, null, 2)}</pre>`;
  ...
  updateData(value, forceValidate = false) {
    if (!forceValidate) {
      ...
      else if (inputElement instanceof Search)
    ...
    const path =
      typeof fullPath === "string" ? fullPath.split("-") : [...fullPath];
  ...
  render() {
    ...
    const descriptionHTML = description
      ? html`<p class="guided--text-input-instructions">
          ${unsafeHTML(capitalize(description))}${[".", "?", "!"].includes(
    ...
    return html`
      <div
        class="${this.required || this.conditional ? "required" : ""} ${this
          .conditional
          ? "conditional"
          : ""}"
      >
  ...
  #onThrow = (...args) =>
    this.onThrow ? this.onThrow(...args) : this.form?.onThrow(...args);
  ...
  #mapToList({ value = this.value, schema = this.schema, list } = {}) {
    ...
    if (canAddProperties) {
      const editable = getEditableItems(this.value, this.pattern, {
        name,
        schema,
      });
          ...
          controls: [
            new Button({
              ...
              onClick: () =>
                this.#createModal({
                  key,
                  schema: isAdditionalProperties(this.pattern)
                    ? undefined
                    : schema,
                  results: value,
                  list: list ?? this.#list,
  ...
  #createModal({ key, schema = {}, results, list, label } = {}) {
    ...
    const createNewObject =
      !results && (schemaCopy.type === "object" || schemaCopy.properties);
    ...
    submitButton.onClick = async () => {
      ...
      if (schemaCopy?.format && schemaCopy.properties) {
        ...
        for (let key in schemaCopy.properties)
      ...
      // Skip if not unique
      if (
        schemaCopy.uniqueItems &&
        list.items.find((item) => item.value === value)
      )
  ...
  #handleNextInput = (idx) => {
    ...
    if (next) {
      ...
      if (firstFocusableElement) {
        if (firstFocusableElement.tagName === "BUTTON")
          return this.#handleNextInput(idx + 1);
  ...
  #moveToNextInput = (ev) => {
    if (ev.key === "Enter") {
      ev.preventDefault();
      if (this.form?.inputs) {
        const idx = Object.values(this.form.inputs).findIndex(
          (input) => input === this,
        );
  ...
  #render() {
    ...
    const itemSchema = this.form?.getSchema
    ...
    if (isArray || canAddProperties) {
      ...
      if (isArray) {
        ...
        if (!("type" in schema.items) && !hasItemsRef) {
          ...
          if (this.value) {
            ...
            schema.items.type = itemToCheck
              ? this.#getType(itemToCheck)
              : "string";
          }
      ...
      // Create tables if possible
        ...
        editableInline.includes(itemSchema?.type) &&
        ...
        const list = new List({
          ...
          onChange: ({ items }) => {
            this.#updateData(
              fullPath,
              items.length ? items.map((o) => postprocess(o.value)) : undefined,
            );
        ...
        if (itemSchema.enum) {
          ...
          input.placeholder =
            schema.placeholder ?? "Provide an item for the list";
        ...
        const externalPath = this.form?.base
          ? [...this.form.base, ...resolvedFullPath]
          : resolvedFullPath;
        ...
        onChange: async ({ object, items }, { object: oldObject }) => {
          if (this.pattern) {
            ...
            const updatedKeys = newKeys.filter(
              (k) => oldObject[k] !== object[k],
            );
            removedKeys.forEach((k) =>
              this.#updateData([...fullPath.slice(1), k], undefined),
            );
            updatedKeys.forEach((k) =>
              this.#updateData([...fullPath.slice(1), k], object[k]),
            );
          } else {
            this.#updateData(
              fullPath,
              items.length ? items.map((o) => o.value) : undefined,
            );
          }
    ...
    if (schema.enum && schema.enum.length) {
      ...
      if (schema.strict && schema.search !== true) {
        return html`
          <select
            ...
            @input=${(ev) =>
              this.#updateData(fullPath, schema.enum[ev.target.value])}
            @change=${() =>
              validateOnChange && this.#triggerValidation(name, path)}
          >
      ...
      search.onchange = () =>
        validateOnChange && this.#triggerValidation(name, path); // Ensure validation on forced change
      ...
      // Handle other string formats
        ...
        return html`
          <input
            class="guided--input schema-input ${schema.step === null
              ? "hideStep"
              : ""}"
            type="${type}"
            ...
            @input=${(ev) => {
              ...
              if (isNumber) {
                ...
                if (
                  ...
                  newValue <= schema.exclusiveMinimum
                ) {
                  if (isStrict)
                ...
                // exclusiveMaximum
                  ...
                  newValue >= schema.exclusiveMaximum
                ) {
                  if (isStrict)
                    newValue = this.value; // Set back to last value
              ...
              if (schema.transform)
                newValue = schema.transform(newValue, this.value, schema);
              ...
              if (isRequiredNumber) {
                const nanHandler =
                  ev.target.parentNode.querySelector(".nan-handler");
                if (!(newValue && Number.isNaN(newValue)))
                  nanHandler.checked = false;
              }
...
customElements.get("jsonschema-input") ||
  customElements.define("jsonschema-input", JSONSchemaInput);
