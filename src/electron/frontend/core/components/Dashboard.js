...
export class Dashboard extends LitElement {
  ...
  attributeChangedCallback(key, _, latest) {
    ...
    if (this.sidebar && (key === "name" || key === "logo"))
      ...
      while (latest && !this.pagesById[latest])
        latest = latest.split("/").slice(0, -1).join("/"); // Trim off last character until you find a page
  ...
  getPage(entry) {
    ...
    if (page instanceof HTMLElement) return page;
      ...
      return this.getPage(Object.values(page)[0]);
  }

  updateSections(
    { sidebar = true, main = false, header = false } = {},
    globalState = this.page.info.globalState,
  ) {
    const info = this.page.info;
    let parent = info.parent;

    if (sidebar) {
      this.subSidebar.sections = this.#getSections(
        parent.info.pages,
        globalState,
      ); // Update sidebar items (if changed)
    }
  ...
  setMain(page) {
    ...
    if (previous) {
      ...
      if (previous.info.globalState)
        toPass.globalState = previous.info.globalState; // Pass global state over if appropriate
    ...
    this.page
      ...
      .then(async () => {
        ...
        const { skipped } =
          this.subSidebar.sections[info.section]?.pages?.[info.id] ?? {};
          ...
          return (
            Promise.all(
              Object.entries(page.workflow).map(async ([_, state]) => {
                if (typeof state.skip === "function" && !backwards)
                  return await state.skip(); // Run skip functions
              }),
            )
      ...
      .finally(() => {
        if (this.#transitionPromise.value)
          this.#transitionPromise.trigger(this.main.page); // This ensures calls to page.to() can be properly awaited until the next page is ready
      });
  }
  ...
  #getSections = (pages = {}, globalState = {}) => {
    ...
    Object.entries(pages).forEach(([id, page]) => {
      ...
      if (info.section) {
        ...
        pageState.skipped = checkIfPageIsSkipped(
          page,
          globalState.project?.workflow,
        );
        ...
        if (id === this.page.info.id)
          state.active = pageState.visited = pageState.active = true; // Set active page as visited
      }
    });
  ...
  #updated(pages = this.pages) {
    ...
    if (isElectron || isStorybook)
      active = new URLSearchParams(url.search).get("page");
    ...
    this.main.onTransition = async (transition) => {
      ...
      if (typeof transition === "number") {
        ...
        if (sign === 1) transition = info.next.info.id;
          ...
          transition = (info.previous ?? info.parent).info.id; // Default to back in time
      }
  ...
  render() {
    ...
    if ("renderNameInSidebar" in this)
      this.sidebar.renderName = this.renderNameInSidebar;
...
customElements.get("nwb-dashboard") ||
  customElements.define("nwb-dashboard", Dashboard);
