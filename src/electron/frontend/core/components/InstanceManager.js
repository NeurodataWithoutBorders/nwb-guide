...
export class InstanceManager extends LitElement {
  ...
  updateState = (id, state) => {
    ...
    for (let i = 0; i < path.length; i++) {
      ...
      if (accordion)
        accordion.setStatus(
          checkStatus(false, false, [...Object.values(target)]),
        );
    }
  };
  ...
  updated = () => {
    ...
    if (selected.length > 0)
      selected.slice(1).forEach((element) => {
        ...
        this.shadowRoot
          .querySelector(`div[data-instance="${instance}"]`)
          .setAttribute("hidden", "");
      });
  ...
  #hideAll(chosenInstanceElement) {
    Array.from(this.shadowRoot.querySelectorAll("div[data-instance]")).forEach(
      (instanceElement) => {
        if (instanceElement !== chosenInstanceElement)
          instanceElement.hidden = true;
      },
    );
  }
  ...
  #render(toRender = this.instances, path = []) {
    ...
    const list = html`
      ${Object.entries(instances).map(([key, info], i) => {
        if (info instanceof HTMLElement || typeof info === "function")
          info = { content: info };
        const listItemInfo = {
          id: key,
          label: key.split("/").pop(),
        ...
        item.onClick = () => {
          ...
          const selectedInstanceElement = instances.find(
            (instanceElement) =>
              instanceElement.getAttribute("data-instance") === this.#selected,
          );
          ...
          <nwb-button size="small" primary @click=${() => {
            ...
            if (input.value) {
              try {
                ...
                let resolvedValue =
                  res instanceof HTMLElement ? res : res?.value ?? null;

                let key = resolvedPath.pop();

                let target = this.instances;
                resolvedPath.forEach(
                  (key) => (target = target[key] = target[key] ?? {}),
                );
...
customElements.get("nwb-instance-manager") ||
  customElements.define("nwb-instance-manager", InstanceManager);
