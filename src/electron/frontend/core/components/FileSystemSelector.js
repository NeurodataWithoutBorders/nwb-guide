...
function getObjectTypeReferenceString(type, multiple, { nested, native } = {}) {
  if (Array.isArray(type))
    return `${multiple ? "" : "a "}${type
      .map((type) =>
        getObjectTypeReferenceString(type, multiple, { native, nested: true }),
      )
      .join(" / ")}`;
...
export class FilesystemSelector extends LitElement {
  ...
  #onThrow = (title, message) => {
    message = message
      ? `<h5 style="margin-bottom: 0;">${title}</h5> <small>${message}</small>`
      : title;
      ...
      options.filters = [
        {
          ...
          extensions: this.accept.map((ext) =>
            ext[0] === "." ? ext.slice(1) : ext,
          ),
        },
  ...
  #check = (value) => {
    // Check type
    const isLikelyFile = fs
      ? fs.statSync(value).isFile()
      : value.split(".").length;
    if (
      (this.type === "directory" && isLikelyFile) ||
      (this.type === "file" && !isLikelyFile)
    )
      this.#onThrow(
        "Incorrect filesystem object",
        `Please provide a <b>${this.type}</b> instead.`,
      );
  };
  ...
  async selectFormat(type = this.type) {
    if (dialog) {
      ...
      const result = Array.isArray(handles)
  ...
  render() {
    ...
    const objectTypeReference = getObjectTypeReferenceString(
      this.type,
      this.multiple,
    );
    ...
    return html`
      <div>
          ...
          ${resolvedValueDisplay
            ? html`
                ...
                ${dialog
                  ...
                  : html`<br /><small
                        >Cannot get full
                        ${isMultipleTypes ? this.type.join(" / ") : this.type}
                        path${this.multiple ? "s" : ""} on web
                        distribution</small
                      >`}
              `
            : html`<span
                ...
                (this.type === "directory" ||
                  (isMultipleTypes &&
                    this.type.includes("directory") &&
                    !dialog))
                  ? html`<br /><small
                        >Multiple directory support only available using
                        drag-and-drop.</small
                      >`
                  : ""}`}
        </button>
        ...
        <div class="controls">
          ${this.value
            ? html`<div @click=${() => this.#handleFiles()}>
                ${unsafeSVG(restartSVG)}
              </div>`
            : ""}
        </div>
      </div>
            ...
            ${this.type.map(
              (type) =>
                html`<nwb-button primary @click=${() => this.selectFormat(type)}
                  >Select
                  ${getObjectTypeReferenceString(type, this.multiple, {
                    native: true,
                  })}</nwb-button
                >`,
            )}
...
customElements.get("filesystem-selector") ||
  customElements.define("filesystem-selector", FilesystemSelector);
