...
export class SimpleTable extends LitElement {
  ...
  #updateRendered = (force) =>
    force || this.rendered === true
      ? (this.rendered = new Promise(
          (resolve) => (this.#rendered = () => resolve((this.rendered = true))),
        ))
      : this.rendered;
  ...
  constructor({
    ...
    document.addEventListener("keydown", (keyDownEvent) => {
      ...
      // Undo / Redo
      if (
        (isMac ? keyDownEvent.metaKey : keyDownEvent.ctrlKey) &&
        keyDownEvent.key === "z"
      )
  ...
  set data(val) {
    this.#data = val;
    this.keyColumn = Array.isArray(this.#data)
      ? undefined
      : this.#keyColumn ?? "Property Key";
  }
  ...
  #clearSelected = (clearFirst = true) => {
    ...
    Object.values(this.#selected).forEach((arr) =>
      arr.forEach((cellElement) =>
        cellElement.parentNode.removeAttribute("selected"),
      ),
    );
    this.#selected = {};
  };
    ...
    const found = path.find((element) => {
      ...
      return (
        ...
        (element instanceof TableCell ||
          element.children?.[0] instanceof TableCell)
      );
    });
    ...
    return cols.map((col, j) => {
      ...
      if (col === this.keyColumn) {
        ...
        if (this.#isUndefined(value))
          value = this.#itemSchema.properties?.[col]?.default;
  ...
  validate = () => {
    ...
    if (!message) {
      ...
      console.log(errors);
      if (len === 1)
        message = errors[0].title || "An error exists on this table.";
      else if (len) {
  ...
  addRow = async (anchorRow = this.#cells.length - 1, n) =>
    (await this.#updateRows(anchorRow, 1))[0];
  ...
  #updateContextMenuRendering = () => {
    ...
    const removeRowButton =
      this.#context.shadowRoot.querySelector("#remove-row");
    ...
    if (!this.#loaded) {
      ...
      body.append(
        ...Object.values(this.#cells).map((row) => {
          ...
          tr.append(
            ...Object.values(row).map((cell, j) => this.#renderCell(cell)),
          );
          return tr;
        }),
      );
      ...
      const mapped = this.mapCells((c) =>
        this.#switchDisplay(c, scrollRoot, false),
      ).flat();

      let filtered = mapped.filter((c) => c);
      scrollRoot.onscroll = () => {
        filtered = filtered
          .map((c) => this.#switchDisplay(c, scrollRoot))
          .filter((c) => c);
      ...
      console.warn(
        `Visible Cell Load Time: ${(performance.now() - tStart).toFixed(2)}ms`,
      );
  ...
  updated() {
    ...
    const cells = data
      .map((row, i) => row.map((v, j) => this.#createCell(v, { i, j })))
      .flat();
  ...
  #updateRows(row, nRows) {
    ...
    after.forEach((element, i) => {
      ...
      Array.from(element.children).forEach(
        (element) => (element.children[0].simpleTableInfo.i = pos),
      ); // Increment position
    });
  ...
  #renderCell = (value, info) => {
    ...
    const cell =
      value instanceof TableCell ? value : this.#createCell(value, info);
  ...
  render() {
    ...
    if (
      this.#itemSchema.additionalProperties !== false ||
      this.#itemSchema.patternProperties
    ) {
    ...
    const genericDescription = rowEditOptions.length
      ? `Right click to ${rowEditOptions.join(" or ")} rows.`
      : "";
          ...
          <thead>
            <tr>
              ${[...this.colHeaders]
                .map(header)
                .map((str, i) =>
                  this.#renderHeader(str, entries[this.colHeaders[i]]),
                )}
            </tr>
          </thead>
          <tbody></tbody>
...
customElements.get("nwb-simple-table") ||
  customElements.define("nwb-simple-table", SimpleTable);
