...
export class Sidebar extends LitElement {
  ...
  updated() {
    ...
    // Toggle sidebar
    const toggle = (this.toggle = (this.shadowRoot ?? this).querySelector(
      "#sidebarCollapse",
    ));
    toggle.onclick = () => {
    ...
    let selectedItem = this.#selected
      ? (this.shadowRoot ?? this).querySelector(
          `ul[data-id='${this.#selected}']`,
        )
  ...
  selectItem = (id) => {
    ...
    const a = (this.shadowRoot ?? this).querySelector(
      `a[data-id="${this.#selected}"]`,
    );
                ...
                ${(() => {
                  ...
                  Object.entries(this.pages).forEach(([id, page]) => {
                    ...
                    const parent = info.group
                      ? groups[info.group] ??
                        (groups[info.group] = document.createElement("div"))
                      : ul;
                    parent.append(li);
                  });
...
customElements.get("nwb-sidebar") ||
  customElements.define("nwb-sidebar", Sidebar);
