...
export class DandiResults extends LitElement {
  ...
  async updated() {
    ...
    const info = await dandiset.getInfo({
      type,
    ...
    const publications = (info.relatedResource ?? []).filter(
      ({ relation }) => relation === "dcite:IsDescribedBy",
    );
        ...
        ...(await Promise.all(
          publications.map(async ({ identifier }) => {
            ...
            li.innerHTML = `${message.author
              ...
              .join(
                ", ",
              )} (${message.created["date-parts"][0][0]}). ${message.title[0]}. <i>${
