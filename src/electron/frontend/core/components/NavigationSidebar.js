...
export class NavigationSidebar extends LitElement {
  ...
  updated() {
    ...
    Object.entries(this.sections).map(([sectionName, info]) => {
      ...
      const isAllSkipped = Object.values(info.pages).every(
        (state) => state.skipped,
      );
      this.#updateClass(
        "skipped",
      ...
      else if (info.open === autoOpenValue)
        this.#toggleDropdown(sectionName, false);
  ...
  #toggleDropdown = (sectionName, forcedState) => {
    ...
    for (const child of children)
      child.toggleAttribute("hidden", hasForce ? !forcedState : undefined);
    ...
    this.#updateClass(
      "fa-chevron-down",
      chevron,
      hasForce ? !forcedState : forcedState,
    );
...
customElements.get("nwb-navigation-sidebar") ||
  customElements.define("nwb-navigation-sidebar", NavigationSidebar);
