...
export const sanitize = (item, condition = isPrivate) => {
  if (isObject(item)) {
    ...
    item.forEach((value) => sanitize(value, condition));
  ...
  for (const [k, value] of Object.entries(toMerge)) {
    ...
    if (
      ...
      if (arrayMergeMethod === "append")
        ...
        target[k] = targetValue.map((targetItem, i) =>
          merge(value[i], targetItem, mergeOptions),
        ); // Merge array entries
      }
      ...
      if (isObject(targetValue))
        target[k] = merge(value, targetValue, mergeOptions);
      else {
...
export function mapSessions(callback = (value) => value, toIterate = {}) {
  return Object.entries(toIterate)
    .map(([subject, sessions]) => {
      return Object.entries(sessions).map(([session, info], i) =>
