...
const deleteIfExists = (path) =>
  fs.existsSync(path) ? fs.rmSync(path, { recursive: true }) : "";
...
function saveNewPipelineFromYaml(name, info, rootFolder) {
  ...
  resolvedMetadata.__generated = structuredClone(
    info.interfaces ? info.metadata ?? {} : {},
  );
...
export class SettingsPage extends Page {
  ...
  generateTestData = async () => {
    ...
    this.notify(
      `Test dataset successfully generated at ${sanitizedOutputPath}!`,
    );
  ...
  updated() {
    ...
    onUpdateAvailable((updateInfo) => {
      ...
      infoIcon.onclick = () => {
        if (this.#releaseNotesModal)
          return (this.#releaseNotesModal.open = true);
  ...
  render() {
    ...
    const generatePipelineButton = new Button({
      ...
      onClick: async () => {
        ...
        const resolved = pipelineNames.reverse().map((name) => {
          try {
            saveNewPipelineFromYaml(
              name,
              examplePipelines[name],
              testing_data_folder,
            );
        ...
        const nSuccessful = resolved.reduce(
          (acc, v) => (acc += v.success === true ? 1 : 0),
          0,
        );
    ...
    setTimeout(() => {
      const testFolderInput = this.form.getFormElement([
        "developer",
        "testing_data_folder",
      ]);
      ...
      >
        ...
        <div>
          ...
          <div style="display: flex; gap: 10px; align-items: center;">
            ...
            fs.existsSync(DATA_OUTPUT_PATH)
              ? [
                  new Button({
                    icon: deleteSVG,
                    label: "Delete",
                    ...
                    onClick: async () => {
                      this.deleteTestData();
                      this.notify(
                        `Test dataset successfully deleted from your system.`,
                      );
                      this.requestUpdate();
                    },
                  }),
                    ...
                    onClick: async () => {
                      if (electron.ipcRenderer) {
                        if (fs.existsSync(DATASET_OUTPUT_PATH))
                          ...
                          this.notify(
                            "The test dataset no longer exists!",
                            "warning",
                          );
                          this.requestUpdate();
                        }
...
customElements.get("nwbguide-settings-page") ||
  customElements.define("nwbguide-settings-page", SettingsPage);
