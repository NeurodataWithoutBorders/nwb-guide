...
export class GuidedHomePage extends Page {
  ...
  resetRadioButtons(parentPageID) {
    const parentPage = (this.shadowRoot ?? this).querySelector(
      `#${parentPageID}`,
    );
    ...
    for (const guidedRadioButton of guidedRadioButtons) {
      ...
      if (elementButtonControls) {
        const elementToHide = (this.shadowRoot ?? this).querySelector(
  ...
  renderProgressCards = (progressFileJSONdata) => {
    ...
    const uploadedList = htmlBase.querySelector(
    ...
    const resumeList = htmlBase.querySelector(
    ...
    const radioButtons = Array.from(
      htmlBase.querySelectorAll(".guided--radio-button"),
    );
  ...
  #onRadioClick = (selectedButton, notSelectedButton) => {
    ...
    if (selectedButton.getAttribute("data-prevent-radio-handler") === true)
      return;
    ...
    if (nextQuestionID) {
      const nextQuestionElement = (this.shadowRoot ?? this).querySelector(
        `#${nextQuestionID}`,
      );
  ...
  resume = (resumeProgressButton) => {
    ...
    const datasetNameToResume =
      resumeProgressButton.parentNode.parentNode.querySelector(
        ".progress-file-name",
      ).innerText;
  ...
  async updated() {
    ...
    const lottieContainer = htmlBase.querySelector(
      "#new-dataset-lottie-container",
    );
    ...
    const datasetCardsRadioButtonsContainer = htmlBase.querySelector(
    ...
    if (guidedSavedProgressFiles.length != 0) {
      ...
      htmlBase
        .querySelector("#guided-button-view-datasets-in-progress")
        .click();
    } else {
      ...
      >
        <div style="padding-bottom: 20px;">
          <h3 style="margin-bottom: 0; padding-bottom: 0;">
            Your one-stop tool for converting data to NWB and uploading it to
            the DANDI Archive.
          </h3>
          <small
        ...
        <div class="create-button" @click="${() => this.to(1)}">
          <div
            id="new-dataset-lottie-container"
            style="height: 150px; width: 150px"
          ></div>
          <h2 class="guided--text-sub-step" style="width: 100%;">
        ...
        <div style="max-width: 800px; width: 100%;">
          ...
          >
            <h2
              ...
              id="guided-continue-curation-header"
            ></h2>
            <div
          ...
          <div
            ...
            hidden
            id="guided-div-resume-progress-cards"
          ></div>
          <div
            class="guided--section"
            hidden
            id="guided-div-update-uploaded-cards"
          ></div>
        </div>
      </div>
    `;
...
customElements.get("nwbguide-guided-home-page") ||
  customElements.define("nwbguide-guided-home-page", GuidedHomePage);
