...
export class GuidedUploadPage extends Page {
  ...
  beforeSave = () => {
    ...
    const isNewDandiset =
      globalState.upload?.dandiset !== this.localState.dandiset;
  ...
  render() {
    ...
    const state = (this.localState = structuredClone(
      this.info.globalState.upload ?? { info: {} },
    ));
          ...
          controls: {
            dandiset: [
              new Button({
                ...
                onClick: () => {
                  const result = createDandiset.call(this, {
    ...
    promise.then(() => {
      ...
      if (!api_keys || !Object.keys(api_keys).length)
        this.header.controls[0].onClick(); // Add modal and open it
    });
