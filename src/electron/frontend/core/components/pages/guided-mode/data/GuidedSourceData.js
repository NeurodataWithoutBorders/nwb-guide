  ...
  #globalButton = new Button({
    ...
    onClick: () => {
      this.#globalModal.form.results = structuredClone(
        this.info.globalState.project.SourceData ?? {},
      );
        ...
        Object.values(this.forms).map(async ({ subject, session, form }) => {
          ...
          const result = await fetch(`${baseUrl}/neuroconv/metadata`, {
            ...
            .catch((error) => {
              ...
              stillFireSwal = false;
              this.notify(
                `<b>Critical Error:</b> ${error.message}`,
                "error",
                4000,
              );
              throw error;
          ...
          if (info.metadata.__generated) {
            ...
            info.metadata = merge(
              merge(generated, metadata, { arrays: true }),
              info.metadata,
            );
          }
          ...
          if (!schemaGlobal.metadata[subject])
            schemaGlobal.metadata[subject] = {};
          ...
          onClick: async (id) => {
            ...
            modal.footer = new Button({
              ...
              onClick: async () => {
                ...
                const sourceCopy = structuredClone(
                  globalState.results[subject][session].source_data,
                );

                const alignmentInfo =
                  globalState.project.alignment ??
                  (globalState.project.alignment = {});

                const sessionInfo = {
                  interfaces: globalState.interfaces,
                  source_data: merge(
                    globalState.project.SourceData,
                    sourceCopy,
                  ),
...
customElements.get("nwbguide-guided-sourcedata-page") ||
  customElements.define(
    "nwbguide-guided-sourcedata-page",
    GuidedSourceDataPage,
  );
