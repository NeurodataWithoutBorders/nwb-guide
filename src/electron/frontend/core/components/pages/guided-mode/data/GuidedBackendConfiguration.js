...
import {
  resolveBackendResults,
  updateSchema,
} from "../../../../../../../schemas/backend-configuration.schema";
...
export class GuidedBackendConfigurationPage extends ManagedPage {
  ...
  renderInstance = (info) => {
    ...
    if (Object.keys(results).length === 0) {
      ...
      instance.innerText =
        "No configuration options available for this session";
    } else {
      const schema = { type: "object", properties: {} };

      const reorganized = Object.entries(results).reduce(
        (acc, [name, item]) => {
          ...
          splitName.reduce((acc, key, i) => {
            ...
            if (i === lenSplit - 1) {
              const { schema, resolved } = resolveBackendResults(
                itemSchema,
                item,
                itemsize,
              );
      ...
      instance = new JSONSchemaForm({
        ...
        validateOnChange: async (name, _, path, value) => {
          ...
          if (name === "chunk_shape") {
            ...
            const mbUsage = getResourceUsageBytes(
              value,
              itemsizes[path.join("/")],
              1e6,
            );
  ...
  getMissingBackendConfigurations = () => {
    const toRun = this.mapSessions(({ session, subject }) => {
      const sesResult =
        this.info.globalState.results[subject][session].configuration;
  ...
  #getManager = () => {
    ...
    this.instances = this.mapSessions(({ subject, session, info }) => {
      const backend =
        info.configuration.backend ?? this.workflow.file_format.value; // Use the default backend if none is set
      ...
      return this.renderInstance({
        ...
        info: {
          ...
          schema:
            this.info.globalState.schema.configuration[subject][session][
              backend
            ], // Get the schema for the current session
        },
      });
      ...
      .then(async (update) => {
        if (Object.keys(update)) {
          this.mapSessions(({ subject, session, info }) => {
            ...
            if (!sesResults.configuration.results)
              sesResults.configuration.results = {};
            if (!sesResults.configuration.itemsizes)
              sesResults.configuration.itemsizes = {};
...
customElements.get("nwbguide-guided-backend-configuration-page") ||
  customElements.define(
    "nwbguide-guided-backend-configuration-page",
    GuidedBackendConfigurationPage,
  );
