...
export class TimeAlignment extends LitElement {
  ...
  render() {
    ...
    for (let name in timestamps) {
      ...
      const hasCompatibleInterfaces =
        isSortingInterface && metadata[name].compatible.length > 0;
      ...
      const resolvedOptionEntries = hasCompatibleInterfaces
        ? optionsCopy
        : optionsCopy.slice(0, 2);

      const elements = resolvedOptionEntries.reduce(
        (acc, [selected, option]) => {
