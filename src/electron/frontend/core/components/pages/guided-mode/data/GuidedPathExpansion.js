...
export async function autocompleteFormatString(path) {
  ...
  const { base_directory } = path.reduce(
    (acc, key) => acc[key] ?? {},
    this.form.resolved,
  );
  ...
  const description = isFile
    ...
    : schema.properties.folder_path.description;
    ...
    schema: {
      ...
      properties: {
        path: {
          ...
          description:
            description ??
            `Provide an example ${pathType} for the selected interface`,
        },
        subject_id: {
          type: "string",
            ...
            return [
              {
                type: "error",
                message:
                  "This feature does not support symbolic links. Please provide a valid path.",
              },
            ];
  ...
  modal.onClose = async () =>
    notify("Format String Path was not completed.", "error");
...
export class GuidedPathExpansionPage extends Page {
  ...
  header = {
    subtitle:
      "Automatic source data detection for multiple subjects / sessions",
  };

  #initialize = () =>
    (this.localState = merge(this.info.globalState.structure, { results: {} }));
    ...
    locate_data: {
      skip: () => {
        ...
        const existingResults =
          Object.values(Object.values(globalState.results ?? {})[0] ?? {})[0] ??
          {};
  ...
  beforeSave = async () => {
    ...
    for (let key in structure) {
      ...
      if (!entry.base_directory && globalBaseDirectory)
        entry.base_directory = globalBaseDirectory;
    ...
    if (subjects.length === 0) {
      ...
      const message =
        "No subjects found with the current configuration. Please try again.";
      this.#notification = this.notify(message, "error");
      throw message;
    }
        ...
        for (let ses in globalResults[sub]) {
          ...
          if (!sesRef)
            ...
            for (let name in globalSesRef.source_data) {
              if (!sesRef.source_data[name])
                delete globalSesRef.source_data[name]; // Delete removed interfaces
            }
          }
        }

        if (Object.keys(globalResults[sub]).length === 0)
          delete globalResults[sub]; // Delete empty subjects
      }
    }
  };
    ...
    const form = (this.form = new JSONSchemaForm({
      ...
      onUpdate: (path, value) => {
        ...
        if (name === "base_directory") {
          ...
          const input = form.getFormElement([
            ...parentPath,
            "format_string_path",
          ]);
      ...
      validateOnChange: async (name, parent, parentPath) => {
        ...
        if (fs) {
          const baseDir = form.getFormElement([
            ...parentPath,
            "base_directory",
          ]);
          if (name === "format_string_path") {
            if (value && baseDir && !baseDir.value) {
              return [
                {
                  message: html`A base directory must be provided to locate your
                  files.`,
                  type: "error",
                },
              ];
            ...
            for (let sub in results) {
              for (let ses in results[sub]) {
                const source_data =
                  results[sub][ses].source_data[interfaceName];
            ...
            if (resolved.length === 0)
              return [
                {
                  message: html`No source files found using the provided
                  information.`,
                  type: "warning",
                },
              ];
...
customElements.get("nwbguide-guided-pathexpansion-page") ||
  customElements.define(
    "nwbguide-guided-pathexpansion-page",
    GuidedPathExpansionPage,
  );
