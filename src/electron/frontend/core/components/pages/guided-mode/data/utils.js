...
export function resolveGlobalOverrides(
  subject,
  globalState,
  resolveMultiSessionOverrides = true,
) {
...
export function drillSchemaProperties(
  schema = {},
  callback,
  target,
  path = [],
  inPatternProperties = false,
) {
...
export function resolveProperties(properties = {}, target, globals = {}) {
  if ("properties" in properties && "type" in properties)
    properties = properties.properties; // Correct for when a schema is passed instead
