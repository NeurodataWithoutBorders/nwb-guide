...
const parentTableRenderConfig = {
  Electrodes: (metadata) => {
    metadata.schema.description =
      "Download, modify, and re-upload data to change the electrode information.";
    return true;
  },
  Units: (metadata) => {
    metadata.truncated = true;
    metadata.schema.description =
      "Update unit information directly on your source data.";
    return true;
  },
};
...
const tableRenderConfig = {
  ...
  ElectrodeColumns: function (metadata) {
    const aggregateRequirements = getAggregateRequirements.call(this, [
      "Ecephys",
      "Electrodes",
    ]);
...
export class GuidedMetadataPage extends ManagedPage {
  ...
  #globalButton = new Button({
    ...
    onClick: () => {
      this.#globalModal.form.results = structuredClone(
        this.info.globalState.project,
      );
  ...
  createForm = ({ subject, session, info }) => {
    ...
    const aggregateGlobalMetadata = resolveGlobalOverrides(
      subject,
      globalState,
      hasMultipleSessions,
    );
    ...
    const schema = preprocessMetadataSchema(
      globalState.schema.metadata[subject][session],
    );
    ...
    const form = new JSONSchemaForm({
      ...
      onOverride: (name) => {
        this.notify(
          `<b>${header(name)}</b> has been overridden with a global value.`,
          "warning",
          3000,
        );
      },
      ...
      transformErrors: (error) => {
        ...
        if (
          error.message.includes('does not conform to the "date-time" format.')
        )
          return false;
        if (
          error.message.includes(
            'not allowed to have the additional property "Ecephys".',
          )
        )
          return false; // NOTE: Remove after including Ecephys metadata
      },
      ...
      onStatusChange: (state) =>
        this.manager.updateState(`sub-${subject}/ses-${session}`, state),
      ...
      renderCustomHTML: function (
        ...
        if (name === "TwoPhotonSeries" && (!this.value || !this.value.length))
          return null;
        if (name === "Device" && (!this.value || !this.value.length))
          return null;
        if (name === "ElectrodeGroup" && (!this.value || !this.value.length))
          return null;
        ...
        if (isAdditional || isPattern) {
          ...
          if (
            additionalPropertiesToRetitle.includes(this.form.base.join("."))
          ) {
            inputSchema.title = "";
            ...
            return html`<div style="width: 100%;">
              ${Object.entries(data)
                .map(([name, value]) => {
                  ...
                  return Object.entries(schemaCopy.patternProperties).map(
                    ([pattern, schema]) => {
      ...
      renderTable: function (name, metadata, fullPath) {
        ...
        const tableConfig =
          tableRenderConfig[name] ??
          parentTableRenderConfig[parentName] ??
          tableRenderConfig["*"] ??
          true;
              ...
              [
                {
                  ...
                  globalState: merge(
                    this.localState,
                    structuredClone(this.info.globalState),
                  ),
                },
              ],
