...
export class GuidedSubjectsPage extends Page {
  ...
  #globalButton = new Button({
    ...
    onClick: () => {
      this.#globalModal.form.results = structuredClone(
        this.info.globalState.project.Subject ?? {},
      );
  ...
  beforeSave = () => {
    ...
    const sourceDataObject = Object.keys(
      this.info.globalState.interfaces,
    ).reduce((acc, key) => {
      acc[key] = {};
      return acc;
    }, {});
    ...
    const modal = (this.#globalModal = createGlobalFormModal.call(this, {
      ...
      formProps: {
        validateOnChange: (localPath, parent, path) => {
          return validateOnChange.call(this, localPath, parent, [
            "Subject",
            ...path,
          ]);
        },
  ...
  render() {
    ...
    const subjects = (this.localState = structuredClone(
      this.info.globalState.subjects ?? {},
    ));

    // Ensure all the proper subjects are in the global state
    const toHave = Object.keys(this.info.globalState.results);
    const toRemove = Object.keys(subjects).filter(
      (sub) => !toHave.includes(sub),
    );
    ...
    for (let subject in subjects) {
      ...
      subjects[subject].identifier = this.#originalState[subject].identifier =
        Symbol("subject"); // Add identifier to subject
    }
    ...
    this.table = new Table({
      ...
      validateOnChange: function (localPath, parent, v) {
        ...
        if (name === "sessions") {
          ...
          return validateOnChange.call(
            this,
            name,
            parent,
            ["Subject", ...localPath.slice(0, -1)],
            v,
          );
        }
      },
