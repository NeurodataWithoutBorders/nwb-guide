...
const skipError = (message) =>
  message.includes("requires") && message.includes("name");
...
export class GuidedNewDatasetPage extends Page {
  ...
  header = {
    subtitle:
      "Enter a name for this pipeline and specify the base folders to save all outputs to",
  };
  ...
  footer = {
    onNext: async () => {
      ...
      if (!name) {
        ...
        this.#nameNotification = this.notify(
  ...
  updateForm = () => {
    ...
    this.form = new JSONSchemaForm({
      ...
      validateOnChange: async (...args) => {
        ...
        if (!results && args[0] === "name")
          this.#nameNotification && this.dismiss(this.#nameNotification);
        return results;
      },
...
customElements.get("nwbguide-guided-newdataset-page") ||
  customElements.define(
    "nwbguide-guided-newdataset-page",
    GuidedNewDatasetPage,
  );
