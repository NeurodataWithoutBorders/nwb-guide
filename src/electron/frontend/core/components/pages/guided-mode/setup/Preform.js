...
export class GuidedPreform extends Page {
  ...
  header = {
    subtitle:
      "Answer the following questions to simplify your workflow through the GUIDE",
  };
    ...
    this.form = new JSONSchemaForm({
      ...
      validateOnChange: function (name, parent, path, value) {
        dependents[name].forEach((dependent) => {
          ...
          } else if (typeof dependent.condition === "boolean")
            condition = (v) => v == dependent.condition;
          else if (Array.isArray(dependent.condition))
            condition = (v) =>
              dependent.condition.some((condition) => v == condition);
          ...
          if (uniformDeps.every(({ name }) => condition(parent[name]))) {
            ...
            if ("required" in dependent)
              dependentEl.required = dependent.required;
            if ("__cached" in dependent)
              dependentEl.updateData(dependent.__cached);
          }

          // Is set to false
          else {
            if (dependentEl.value !== undefined)
              dependent.__cached = dependentEl.value;
