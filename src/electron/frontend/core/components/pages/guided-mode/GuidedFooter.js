  ...
  render() {
    return html`
      ...
      >
        <div>
          ${this.exit
            ? html`<nwb-button @click=${this.onExit}>${this.exit}</nwb-button>`
            : ""}
        </div>

        <div>
          ${this.back
            ? html`<nwb-button @click=${this.onBack}>${this.back}</nwb-button>`
            : ""}
          ${this.next
            ? html`<nwb-button @click=${this.onNext} primary
                >${this.next}</nwb-button
              >`
            : ""}
        </div>
      </nwb-footer>
...
customElements.get("nwb-guided-footer") ||
  customElements.define("nwb-guided-footer", GuidedFooter);
