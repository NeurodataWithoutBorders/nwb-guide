...
export function schemaToPages(
  ...
  globalStatePath,
  options,
  transformationCallback = (info) => info,
) {
...
export class GuidedFormPage extends Page {
  ...
  render() {
    ...
    const temp = this.info.globalStatePath
      ? this.info.globalStatePath.reduce(
          (acc, key) => acc[key] ?? (acc[key] = {}),
          this.info.globalState,
        )
      : {};

    const results = (this.localState = structuredClone({
      [key]: temp[key] ?? (temp[key] = {}),
    })); // Keep a local copy of the results
...
customElements.get("nwbguide-guided-form-page") ||
  customElements.define("nwbguide-guided-form-page", GuidedFormPage);
