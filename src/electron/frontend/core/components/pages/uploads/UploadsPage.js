...
const dandiSchema = merge(
  dandiUploadSchema,
  structuredClone(dandiStandaloneSchema),
  { arrays: "append" },
);
  ...
  const form = new JSONSchemaForm({
    ...
    validateOnChange: async (name, parent) => {
      ...
      if (name === "nih_award_number") {
        if (value)
          return (
            awardNumberValidator(value) || [
              { type: "error", message: AWARD_VALIDATION_FAIL_MESSAGE },
            ]
          );
          ...
          return [
            {
              type: "error",
              message:
                "You must provide an NIH Award Number to embargo your data.",
            },
          ];
      }
    },
...
async function getAPIKey(staging = false) {
  ...
  if (isInvalid) {
    ...
    modal.onClose = async () =>
      notify("The updated DANDI API key was not set", "error");
...
export async function uploadToDandi(
  info,
  type = "project" in info ? "project" : "",
) {
  const { dandiset } = info;
  ...
  const result = await run(
    type ? `neuroconv/upload/${type}` : "neuroconv/upload",
    payload,
    {
...
export class UploadsPage extends Page {
    ...
    const modal = (this.#globalModal = createFormModal.call(this, {
      ...
      onSave: async (form) => {
        ...
        if (!Object.keys(apiKeys).length) {
          this.#saveNotification = this.notify(
      ...
      formProps: {
        validateOnChange: async (name, parent) => {
          ...
          if (name.includes("api_key"))
            return await validateDANDIApiKey(value, name.includes("staging"));
        },
      },
    }));
    document.body.append(modal);
  ...
  render() {
        ...
        return (this.form = new JSONSchemaForm({
          ...
          transformErrors: (error) => {
            if (error.message === "Filesystem Paths is a required property.")
              error.message =
                "Please select at least one file or folder to upload.";
          },
    ...
    promise.then(() => {
      ...
      if (!api_keys || !Object.keys(api_keys).length)
        this.#globalModal.open = true;
    });
...
customElements.get("nwbguide-uploads-page") ||
  customElements.define("nwbguide-uploads-page", UploadsPage);
