...
export class InspectPage extends Page {
  ...
  inspect = async (paths, kwargs = {}, options = {}) => {
    ...
    if (!messages.length)
      return this.notify("No messages received from the NWB Inspector");
  ...
  showReport = async (value) => {
    ...
    const items = truncateFilePaths(
      messages,
      getSharedPath(messages.map((item) => item.file_path)),
    );
    ...
    schema: {
      properties: {
        ...
        n_jobs: {
          ...
          title: "Job Count",
          description:
            "Number of parallel jobs to run. Leave blank to use all available cores.",
          min: 1,
          step: 1,
        },
      },
  ...
  render() {
    const button = new Button({
      ...
      onClick: async () =>
        this.showReport(this.form.inputs["filesystem_paths"].value),
    });
...
customElements.get("nwbguide-inspect-page") ||
  customElements.define("nwbguide-inspect-page", InspectPage);
