...
export class Page extends LitElement {
  ...
  load = (
    datasetNameToResume = new URLSearchParams(window.location.search).get(
      "project",
    ),
  ) => (this.info.globalState = get(datasetNameToResume));

  addSession({ subject, session, info }) {
    if (!this.info.globalState.results[subject])
      this.info.globalState.results[subject] = {};
  ...
  mapSessions = (callback, data = this.info.globalState.results) =>
    mapSessions(callback, data);
  ...
  async convert({ preview, ...conversionOptions } = {}, options = {}) {
    ...
    if (preview) {
      if (!options.title)
        options.title = "Running preview conversion on all sessions...";

      const stubs = await this.runConversions(
        { stub_test: true, ...conversionOptions },
        undefined,
        options,
      );
      ...
      this.info.globalState[key] = await this.runConversions(
        conversionOptions,
        true,
        options,
      );
  ...
  async runConversions(
    ...
    if (typeof original === "number")
      ...
      toRun = toRun.filter(({ subject }) => subject === original);
    ...
    try {
      for (let info of toRun) {
        ...
        const { conversion_output_folder, name, SourceData, alignment } =
          globalState.project;
        ...
        if (!configurationCopy.backend)
          configurationCopy.backend = this.workflow.file_format.value;
        ...
        if (optsCopy.configuration === false) {
          ...
          if (typeof optsCopy.configuration === "object")
            merge(optsCopy.configuration, configurationCopy);
        }

        delete optsCopy.configuration;

        const payload = {
          output_folder: optsCopy.stub_test
            ? undefined
        ...
        if (hasCustomFunction) {
          ...
          const subRef =
            conversionOutput[subject] ?? (conversionOutput[subject] = {});
      ...
      if (fileConfiguration.length) {
        const results = await run(
          ...
          },
          {
            ...
            onError: () =>
              "Conversion failed with current metadata. Please try again.",
            ...swalOpts,
          },
        ).catch(async (error) => {
        ...
        results.forEach((info) => {
          ...
          const [subject, session] = fileName
            .match(/sub-(.+)_ses-(.+)\.nwb/)
            .slice(1);
          const subRef =
            conversionOutput[subject] ?? (conversionOutput[subject] = {});
  ...
  updateSections = () => {
    ...
    dashboard.updateSections(
      { sidebar: true, header: true },
  ...
  set unsavedUpdates(value) {
    ...
    if (value === "conversions")
      this.info.globalState.desyncedData = { preview: true, conversion: true };
  }
...
customElements.get("nwbguide-page") ||
  customElements.define("nwbguide-page", Page);
