...
export class Table extends LitElement {
  ...
  #getRowData(row, cols = this.colHeaders) {
    ...
    return cols.map((col, j) => {
      ...
      if (col === this.keyColumn) {
        ...
        value = (hasRow ? this.data[row][col] : undefined) ?? this.globals[col];
  ...
  validate = () => {
    ...
    if (!message) {
      ...
      if (len === 1)
        message = errors[0].getAttribute("data-message") || "Error found";
  ...
  getRowName = (row) =>
    this.keyColumn
      ? Object.entries(this.data).find(([k, v]) => v[rowSymbol] === row)?.[0]
      : row;

  revalidate = (skipped = []) => {
    this.table.getData().forEach((rowData, i) => {
      rowData.forEach((value, j) => {
        const isSkipped = skipped.find(
          ({ row, prop }) => row === i && j === prop,
        );
  ...
  updated() {
    ...
    Object.keys(this.data).forEach((row, i) =>
      Object.defineProperty(this.data[row], rowSymbol, {
        value: i,
        configurable: true,
      }),
    ...
    const columns = colHeaders.map((k, i) => {
      ...
      if (colInfo.unit)
        displayHeaders[i] = `${displayHeaders[i]} (${colInfo.unit})`;

      // Enumerate Possible Values
      if (colInfo.enum) {
        info.source = colInfo.enumLabels
          ? Object.values(colInfo.enumLabels)
          : colInfo.enum;
      ...
      const validator = async function (value, callback) {
        ...
        if (value && k === instanceThis.keyColumn) {
          if (
            value in instanceThis.data &&
            instanceThis.data[value]?.[rowSymbol] !== row
          ) {
        ...
        if (isUndefined && required) {
          instanceThis.#handleValidationResult(
            [
              {
                message: `${header(k)} is a required property.`,
                type: "error",
              },
            ],
            row,
    ...
    if (this.#itemSchema.additionalProperties)
      contextMenu.push("col_right", "remove_col");

    contextMenu = contextMenu.filter(
      (k) => !(this.contextMenu.ignore ?? []).includes(k),
    );
    ...
    const operationOn = Object.keys(operations).filter(
      (k) => operations[k].length,
    );
        ...
        if (isUserUpdate) {
          ...
          if (header === this.keyColumn) {
            if (isValid && value && value !== rowName) {
              ...
              Object.defineProperty(this.data[value], rowSymbol, {
                value: row,
                configurable: true,
              }); // Setting row tracker
          ...
          // Update data on passed object
            ...
            if (value == undefined || value === "") {
              ...
              target[rowName][header] =
                value === globalValue ? undefined : value;
            }
          }
    ...
    return Object.values(data).map((row) =>
      row.map((value, j) =>
        this.#getRenderedValue(
          value,
          this.#itemSchema.properties[this.colHeaders[j]],
        ),
      ),
    );
  };
  ...
  #getValue = (value, colInfo) => {
    ...
    if (colInfo.enumLabels)
      return (
        Object.keys(colInfo.enumLabels).find(
          (k) => colInfo.enumLabels[k] === value,
        ) ?? value
      );
  ...
  #handleValidationResult = (result, row, prop) => {
    const warnings = Array.isArray(result)
      ? result.filter((info) => info.type === "warning")
      : [];
    const errors = Array.isArray(result)
      ? result?.filter((info) => info.type === "error")
      : [];
  ...
  connectedCallback() {
    ...
    const exists = (this.stylesheet = stylesheets.find(
      (stylesheet) => styleSymbol in stylesheet,
    ));
