...
export class BasicTable extends LitElement {
  ...
  #renderHeaderContent = (str) => {
    const required = this.#itemSchema.required
      ? this.#itemSchema.required.includes(str)
      : false;
    if (required)
  ...
  #renderHeader = (prop, { description, title = prop } = {}) => {
    ...
    const required = this.#itemSchema.required
      ? this.#itemSchema.required.includes(prop)
      : false;
  ...
  #getType = (value, { type, data_type } = {}) => {
    ...
    if (type) {
      ...
      if (resolved.includes("int") || resolved.includes("float"))
        resolved = "number";
  ...
  #validateCell = (value, col, row, parent) => {
    ...
    if (
      isUndefined &&
      "required" in this.#itemSchema &&
      this.#itemSchema.required.includes(col)
    )
  ...
  #getTSV = () => {
    ...
    const sections = [
      ...
      data
        .map((row, i) => {
          const mapped = row.map((col) =>
            typeof col !== "string" ? JSON.stringify(col) : col,
          );
          return mapped.join("\t");
        })
        .join("\n"),
  ...
  render() {
    ...
    return html`
      ...
      ${this.truncated
        ? html`<p
            style="margin: 0; width: 100%; text-align: center; font-size: 150%;"
          >
            ...
          </p>`
        : ""}
...
customElements.get("nwb-basic-table") ||
  customElements.define("nwb-basic-table", BasicTable);
