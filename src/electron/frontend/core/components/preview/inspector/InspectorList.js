...
export class InspectorList extends List {
  ...
  constructor(props = {}) {
    const { items } = props;
    const aggregatedItems = Object.values(aggregateMessages(items)).map(
      (items) => {
        const aggregate = { ...items.pop() }; // Create a base object for the aggregation
        aggregate.files = [
          aggregate.file_path,
          ...items.map(({ file_path }) => file_path),
        ];
        return aggregate;
      },
    );
...
export class InspectorListItem extends LitElement {
  ...
  render() {
    ...
    return html`
      ${hasMetadata
        ? html`<span id="objectType"
            >${hasObjectType ? `${this.object_type}` : ""}
          </span>`
        : ""}
      ${hasMetadata
...
customElements.get("nwbguide-inspector-legend") ||
  customElements.define("nwbguide-inspector-legend", InspectorLegend);
