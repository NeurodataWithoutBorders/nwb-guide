...
export const get = (path, object, omitted = [], skipped = []) => {
  ...
  return path.reduce((acc, curr, i) => {
    const tempAcc =
      acc?.[curr] ??
      acc?.[omitted.find((str) => acc[str] && acc[str][curr])]?.[curr];
...
export class JSONSchemaForm extends LitElement {
  ...
  constructor(props = {}) {
    ...
    this.results =
      (props.base ? structuredClone(props.results) : props.results) ?? {}; // Deep clone results in nested forms
    ...
    this.validateEmptyValues =
      props.validateEmptyValues === undefined
        ? true
        : props.validateEmptyValues; // false = validate when not empty, true = always validate, null = never validate
  ...
  getAllFormElements = (
    path,
    config = { forms: true, tables: true, inputs: true },
  ) => {
    const name = path[0];
    ...
    // Get Single element
      ...
      if (last) {
        ...
        else if (result instanceof JSONSchemaInput && config.inputs)
          return [result];
        ...
        if (result instanceof JSONSchemaForm)
          return result.getAllFormElements(upcomingPath, config);
        else return [result];
      }
    }
  };
  ...
  #getElementOnForm = (
    path,
    { forms = true, tables = true, inputs = true } = {},
  ) => {
    if (typeof path === "string") path = path.split(".");
  ...
  updateData(localPath, value, forceUpdate = false) {
    ...
    if (this.isUndefined(value)) {
      ...
      resolvedParent[name] =
        isObject(value) && isObject(resolvedParent)
          ? merge(value, resolvedParent[name])
          : value; // Merge with existing resolved values
    }
    ...
    return validator
      ...
      .errors.map((e) => {
        const propName =
          e.path.slice(-1)[0] ??
          name ??
          (e.property === "instance" ? "Form" : e.property);
        ...
        if (
          e.name === "required" &&
          this.validateEmptyValues === null &&
          !(e.property in e.instance)
        )
          return;

        // Non-Strict Rule
        if (
          resolvedSchema.strict === false &&
          e.message.includes("is not one of enum values")
        )
          return;
  ...
  validate = async (resolved = this.resolved) => {
    ...
    const flaggedInputs = (
      this.shadowRoot
        ? Array.from(this.shadowRoot.querySelectorAll(".invalid"))
        : []
    ).filter((el) => el.nextElementSibling); // Skip tables
    ...
    const allErrors = flaggedInputs
      .map((inputElement) =>
        Array.from(inputElement.nextElementSibling.children).map(
          (li) => li.message,
        ),
      )
      .flat()
  ...
  #get = (path, object = this.resolved, omitted = [], skipped = []) =>
    get(path, object, omitted, skipped);
  ...
  getSchema = (path, schema = this.schema) =>
    getSchema(path, schema, this.base);
  ...
  #renderInteractiveElement = (
    ...
    const isConditional =
      this.#getLink(externalPath)?.conditional ||
      typeof isRequired === "function"; // Check the two possible ways of determining if a field is conditional

    if (isConditional && !isRequired)
      isRequired = required[name] = async () => {
        const isRequiredAfterChange =
          await this.#checkRequiredAfterChange(localPath);
        if (isRequiredAfterChange) {
          return true;
        } else {
          const linkResults = await this.#applyToLinkedProperties(
            this.#checkRequiredAfterChange,
            localPath,
          ); // Check links
  ...
  checkAllLoaded = () => {
    const expected = [...Object.keys(this.forms), ...Object.keys(this.tables)]
      .length;
  ...
  #validateRequirements = async (
    ...
    for (let name in requirements) {
      ...
      if (typeof isRequired === "function")
        isRequired = await isRequired.call(this.resolved);
      if (isRequired) {
        ...
        // else
        if (
          this.isUndefined(resolved[name]) &&
          this.validateEmptyValues !== null
        )
          invalid.push(path);
      }
  ...
  #getRenderable = (
    ...
    const res = entries
      .map(([key, value]) => {
        ...
        if (this.showLevelOverride >= path.length)
          return isRenderable(key, value);
        if (required[key]) return isRenderable(key, value);
        if (this.#getLink([...this.base, ...path, key]))
          return isRenderable(key, value);
  ...
  #getGroup = (args) => {
    if (typeof args === "string") args = args.split("-");
    const group = this.groups.find((linked) =>
    ...
    return (
      await this.#applyToLinkedProperties((link) => {
        const isRequired = this.#isRequired(
          link.slice((this.base ?? []).length),
        );
        if (typeof isRequired === "function")
          return !isRequired.call(this.resolved);
  ...
  triggerValidation = async (
    ...
    const undefinedPathToken =
      localPath.findIndex((str) => !str && typeof str !== "number") !== -1;
    ...
    const skipValidation =
      this.validateEmptyValues === null && value === undefined;
    ...
    const jsonSchemaErrors =
      validateArgs.length === 2
        ? this.validateSchema(...validateArgs, name)
        : [];

    const valid = skipValidation
      ? true
      : await this.validateOnChange(name, parent, pathToValidate, value);

    if (valid === null) return null; // Skip validation / data change if the value is null

    const isRequired =
      this.#isRequired(localPath) || (!input.table && input.required); // Do not trust required status of table validations

    let warnings = Array.isArray(valid)
      ? valid.filter(
          (info) => info.type === "warning" && (!isRequired || !info.missing),
        )
      : [];
    ...
    const errors = [
      ...(Array.isArray(valid)
        ? valid?.filter(
            (info) => info.type === "error" || (isRequired && info.missing),
          )
        : []), // Derived Errors
    ...
    if (!isValid && errors.length === 0)
      errors.push({ type: "error", message: "Invalid value detected" });

    const resolvedErrors = this.#resolveErrors(errors, externalPath, parent);

    // Track errors and warnings
    const updatedWarnings = warnings
      .map((info) => (onWarning ? onWarning(info) : info))
      .filter((v) => !!v);
    const updatedErrors = resolvedErrors
      .map((info) => (onError ? onError(info) : info))
      .filter((v) => !!v);
    ...
    updatedWarnings.forEach((info) =>
      onWarning ? "" : this.#addMessage(localPath, info, "warnings"),
    );
    info.forEach((info) =>
      onInfo ? onInfo(info) : this.#addMessage(localPath, info, "info"),
    );
    ...
    if (groupEl) {
      ...
      const allErrors = inputs.reduce(
        (acc, id) => acc + parseInt(groupEl.getAttribute(`data-${id}-errors`)),
        0,
      );
      const allWarnings = inputs.reduce(
        (acc, id) =>
          acc + parseInt(groupEl.getAttribute(`data-${id}-warnings`)),
        0,
      );
    ...
    if (clearAllErrors) {
      ...
      if (this.validateEmptyValues) {
        ...
        updatedErrors.forEach((info) =>
          onError ? "" : this.#addMessage(localPath, info, "errors"),
        );
      }
  ...
  #render = (schema, results, required = {}, ignore = {}, path = []) => {
    ...
    let rendered = finalSort.map((entry) => {
      ...
      if (!info.properties)
        return this.#renderInteractiveElement(name, info, required, path);
      ...
      const renderableInside = this.#getRenderable(
        info,
        required[name],
        ignore,
        localPath,
      ...
      const __disabled =
        this.results.__disabled ?? (this.results.__disabled = {});
      const __interacted =
        __disabled.__interacted ?? (__disabled.__interacted = {});

      const hasInteraction = __interacted[name]; // NOTE: This locks the specific value to what the user has chosen...

      const { __disabled: __tempDisabledGlobal = {} } = this.getGlobalValue(
        localPath.slice(0, -1),
      );
      ...
      let isGlobalEffect =
        !hasInteraction ||
        (!hasInteraction && __disabledGlobal.__interacted?.[name]); // Indicate whether global effect is used
      ...
      const nestedResults =
        __disabled[name] ?? results[name] ?? this.results[name]; // One or the other will exist—depending on global or local disabling
    ...
    if (hasPatternProperties) {
      const patternProps = Object.entries(schema.patternProperties).map(
        ([key, schema]) => {
          return this.#renderInteractiveElement(
            key,
            {
    ...
    const additionalProps = getEditableItems(results, additionalPropPattern, {
      schema,
    });
      ...
      Object.entries(schema.properties).forEach(([key, value]) => {
        ...
        if (value.properties || isPropItem) {
          ...
          if (!nextAccumulator || isNotObject)
            nextAccumulator = acc[key] = {
              [selfRequiredSymbol]: !!(nextAccumulator && !isPropItem),
            };
  ...
  get rendered() {
    ...
    this.#resolving = resolve(this.#rendered, () => {
      const promise = Promise.all(
        [...Object.values(this.forms), ...Object.values(this.tables)].map(
          ({ rendered }) => rendered,
        ),
      );
...
customElements.get("nwb-jsonschema-form") ||
  customElements.define("nwb-jsonschema-form", JSONSchemaForm);
