...
export class Search extends LitElement {
  ...
  #close = () => {
    if (
      this.listMode === "input" &&
      this.getAttribute("interacted") === "true"
    ) {
  ...
  updated() {
    ...
    this.#options = Array.from(options).map((option) => {
      ...
      const structuredKeywordString = option.getAttribute(
        "data-structured-keywords",
      );

      const keywords = keywordString
        ? JSON.parse(option.getAttribute("data-keywords"))
        : [];
      const structuredKeywords = structuredKeywordString
        ? JSON.parse(structuredKeywordString)
        : {};
      ...
      return {
        ...
        keywords,
        structuredKeywords,
        label: option.querySelector(".label").innerText,
      };
    });
  ...
  #displayValue = (option) => {
    return (
      ...
      option?.value ??
      option?.key ??
      (this.#isObject(option) ? undefined : option)
    );
  };
  ...
  #initialize = () => {
    this.#options.forEach(({ option }) =>
      option[this.showAllWhenEmpty ? "removeAttribute" : "setAttribute"](
        "hidden",
        "",
      ),
    );
  ...
  #populate = (input = this.value ?? "") => {
    ...
    this.#options.forEach(({ label, keywords, structuredKeywords }, i) => {
      ...
      const allKeywords = [
        ...keywords,
        ...Object.values(structuredKeywords).flat(),
      ];
      const allKeywordTokens = allKeywords
        .map((keyword) => this.getTokens(keyword))
        .flat();
      const allTokens = [...labelTokens, ...allKeywordTokens];

      const result = inputTokens.reduce((acc, token) => {
        for (let subtoken of allTokens) {
          if (subtoken.startsWith(token) && !toShow.includes(i))
            return (acc += 1);
        }
        return acc;
      }, 0);
    ...
    this.#sortedCategories.forEach(({ entries, element }) => {
      if (
        entries.reduce(
          (acc, entryElement) => acc + entryElement.hasAttribute("hidden"),
          0,
        ) === entries.length
      )
  ...
  render() {
    ...
    if (this.disabledLabel)
      this.style.setProperty("--disabled-label", `"${this.disabledLabel}"`);
      ...
      const itemEls = options
        ...
        .map((option) => {
          ...
          if (keywords)
            listItemElement.setAttribute(
              "data-keywords",
              JSON.stringify(option.keywords),
            );
    ...
    const categories = (this.#sortedCategories = Object.values(
      this.categories,
    ).sort((a, b) => {
    ...
    <div class="header" style=${styleMap({
      ...
      <input placeholder=${this.placeholder} value=${valueToDisplay} @click=${(
        clickEvent,
      ) => {
        clickEvent.stopPropagation();
