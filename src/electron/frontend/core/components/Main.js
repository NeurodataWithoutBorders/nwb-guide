...
export class Main extends LitElement {
  ...
  render() {
    ...
    if (page) {
      ...
      if (footer && "onNext" in footer && !("next" in footer))
        footer.next = "Next";
    }

    const footerEl = footer
      ? (this.footer = new GuidedFooter(footer))
      : html`<div></div>`; // Render for grid
...
class MainHeader extends LitElement {
  ...
  render() {
    ...
    if (page) {
      ...
      if (section) {
        if (
          config === true ||
          !("header" in page) ||
          !("sections" in page.header)
        ) {
          const sectionNames = Object.entries(sections)
            .filter(
              ([name, info]) =>
                !Object.values(info.pages).every((state) => state.skipped),
            )
            .map(([name]) => name);

          config =
            page.header && typeof page.header === "object" ? page.header : {};
    ...
    const headerEl = config
      ? (this.header = new GuidedHeader(config))
      : html`<div></div>`; // Render for grid
            ...
            >
              ...
              >
                <div style="line-height: 1em; color: gray;">
                  <h1 class="title" style="margin: 0; padding: 0; color:black;">
                    ${this.title}
                  </h1>
                  <small
                    >${subtitle instanceof HTMLElement
                      ? subtitle
                      : unsafeHTML(subtitle)}</small
                  >
                </div>
                <div style="padding-left: 25px; display: flex; gap: 10px;">
                  ${controls}
                </div>
              </div>
...
customElements.get("nwb-main-header") ||
  customElements.define("nwb-main-header", MainHeader);
