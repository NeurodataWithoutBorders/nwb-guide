...
export function getMessageType(item) {
  return (
    item.type ??
    (isErrorImportance.includes(item.importance) ? "error" : "warning")
  );
}
...
export async function validateOnChange(name, parent, path, value) {
  ...
  if (
    lastResolved !== false &&
    (functions === undefined || functions === true)
  ) {
    const getNestedMatches = (result, searchPath, toAlwaysCheck = []) => {
      ...
      if (searchPath.length)
        toAlwaysCheck.forEach((obj) =>
          matches.push(...getMatches(obj, searchPath, toAlwaysCheck)),
        );
      return matches;
    };
    ...
    const getMatches = (obj = {}, searchPath, toAlwaysCheck = []) => {
      ...
      if (obj["*"])
        matches.push(
          ...getNestedMatches(obj["*"], updateSearchPath, updatedAlwaysCheck),
        );
      ...
      ); // Always search to the end of the search path
      return matches;
    };
      ...
      return flat
        ...
        .map((messageInfo) => {
          return {
            ...
            missing:
              messageInfo.missing ?? messageInfo.message.includes("is missing"), // Indicates that the field is missing
          };
