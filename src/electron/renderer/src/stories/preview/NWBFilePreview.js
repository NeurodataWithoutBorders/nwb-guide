...
class NWBPreviewInstance extends LitElement {
  ...
  render() {
    ...
    fetch(`${baseUrl}/neurosift/files/${this.file}`, { method: "POST" })
      .then((res) => res.text())
      .then((result) => {
    ...
    return html` ${new FullScreenToggle({ target: this })}
      <div style="display: flex; height: 100%;">
        ...
        ${this.inspect
          ? html`<div id="inspect">
              <h3
                style="padding: 10px; margin: 0; background: black; color: white;"
              >
                Inspector Report
              </h3>
              ${until(
                ...
                html`<small style="padding: 10px 25px;"
                  >Loading inspector report...</small
                >`,
              )}
            </div>`
          : ""}
