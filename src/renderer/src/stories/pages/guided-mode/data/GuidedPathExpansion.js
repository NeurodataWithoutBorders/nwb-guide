...
const exampleFormatPath = "Subjects/{subject_id}/{session_start_time:%Y-%m-%d}/{session_id}/raw_video_data/leftCamera.raw.{}.mp4"

const exampleMetadata = {
    "source_data": {
        "IBL_video": {
            "file_path": "/mylab/Subjects/NR_0017/2022-03-22/001/raw_video_data/leftCamera.raw.6252a2f0-c10f-4e49-b085-75749ba29c35.mp4"
        }
    },
    "metadata": {
        "NWBFile": {
...
const pathExpansionInfoBox = new InfoBox({
    ...
    content: html`
        <div>
            <p>Consider a dataset of that includes video recordings from three cameras, stored in the following directory structure.</p>
            ${new CodeBlock({ text: exampleFileStructure })}

            <p>Using <code>mylab</code> as the base directory, the correct format string to extract the subject ID, session start time, and session number would be:</p>
            ${new CodeBlock({ text: exampleFormatPath})}

            <hr>

            <p>The above example applies all of the supported f-string variables, which are used to extract information into the resulting metadata:</p>
            ${new List({
                items: [
                    { 
                        value: 'subject_id'
            ...
            <p>Consequently, the metadata extracted from the first file found using this approach would be the following:</p>
            ${new CodeBlock({ text: JSON.stringify(exampleMetadata, null, 2)})}

            <hr>
            
            <small>For complete documentation of the path expansion feature of NeuroConv, visit the
            ...
            validateOnChange: async (name, parent, parentPath) => {
                ...
                if (fs) {
                    ...
                    if (name === 'format_string_path') {
                        ...
                        const base_directory = [...parentPath, 'base_directory'].reduce((acc, key) => acc[key], this.form.resolved)
                        ...
                        return [{ 
                            message: html`
                            <h4 style="margin-bottom: 0;">Source Files Found</h4>
                            <small><i>Inspect the Developer Console to preview the metadata results</i></small>
                            <small>${new List({
                                items: resolved.map(path => { return { value: path} }),
                                emptyMessage: 'N/A',
                                editable: false
                            })}</small>`,
                            type: 'info'
