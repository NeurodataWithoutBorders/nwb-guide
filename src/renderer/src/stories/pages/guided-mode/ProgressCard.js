...
export class ProgressCard extends LitElement {
    ...
    render() {
        ...
        if (progressFileImage === "") {
            ...
            progressFileImage = html`
                <img src="${progressFileImage}" alt="Dataset banner image" style="height: 80px; width: 80px" />
            `;
        }
        const progressFileName = this.info.project["name"] || "";
        const progressFileSubtitle = this.info.project.NWBFile?.["lab"] || this.info.project.NWBFile?.["institution"];
        ...
        const savedUploadDataProgress =
            this.info["previously-uploaded-data"] && Object.keys(this.info["previously-uploaded-data"]).length > 0;
