    ...
    render() {
        ...
        const generatePipelineButton = new Button({
            ...
            onClick: async () => {
                ...
                if (nFailed) {
                    const failDisplay = nFailed === 1 ? `the <b>${resolved.find(v => typeof v === 'string')}</b> pipeline` : `${nFailed} pipelines`
                    this.#openNotyf(`<h4 style="margin-bottom: 0;">Generated ${nSuccessful} test pipelines.</h4><small>Could not find source data for ${failDisplay}.`, "warning");
                }
                else if (nSuccessful) this.#openNotyf(`Generated ${nSuccessful} test pipelines.`, "success");
                else this.#openNotyf(`<h4 style="margin-bottom: 0;">Pipeline Generation Failed</h4><small>Could not find source data for any pipelines.</small>`, 'error')
            },
        });
