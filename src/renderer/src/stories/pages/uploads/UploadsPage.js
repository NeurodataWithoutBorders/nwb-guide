...
export class UploadsPage extends Page {
    ...
    render() {
        ...
        let options = getEntries().map(str => str.slice(0, -5)).map(str => projectPrefix + str)
        ...
        if (reloading) latestProjectInfo = reloadingCustomFolder ? this.getCustomFolderInfo(customFolderDisplay.innerText = globalState.nwb_folder_path) : get(globalState.project)
            ...
            onUpdate: async (value) => {
                if (value) {
                    ...
                    if (value === 'Select a custom folder') {
                        ...
                        const folder = globalState.nwb_folder_path = customFolderDisplay.innerText = result.filePath ?? result.filePaths?.[0]
        ...
        <p>
            This page allows you to upload projects with converted files to the DANDI Archive.
        </p>
        <hr />
