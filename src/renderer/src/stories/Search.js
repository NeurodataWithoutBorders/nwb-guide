...
export class Search extends LitElement {
  ...
  #initialize = () =>
    this.#options.forEach(({ option }) =>
      option.classList[this.showAllWhenEmpty ? "remove" : "add"]("hidden")
    );
    ...
    <div class="header">
      <input placeholder="Type here to search" @input=${(ev) => {
        ...
        this.#options.forEach(({ option, label }, i) => {
          if (label.toLowerCase().includes(input.toLowerCase()) && !toShow.includes(i))
            toShow.push(i);
        });
        ...
        this.#options.forEach(({ option, keywords }, i) => {
          keywords.forEach((keyword) => {
            if (keyword.toLowerCase().includes(input.toLowerCase()) && !toShow.includes(i))
              toShow.push(i);
          });
        });
