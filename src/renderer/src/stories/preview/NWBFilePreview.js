...
export class NWBFilePreview extends LitElement {
    ...
    render() {
        ...
        const fileArr = Object.entries(this.files).map(([subject, v]) => Object.entries(v).map(([session, info]) => { return { subject, session, info } })).flat()
        ...
        return html`<div style="display: flex; height: 100%;">
                ...
                (async () => {
                    ...
                    const items = onlyFirstFile ? await run("inspect_nwbfile", { nwbfile_path: fileArr[0].info.file, ...opts }) // Inspect the first file 
                    : await (async () => {
