    ...
    render() {
        ...
        return html`<div style="display: flex; height: 100%;">
            <div style="flex-grow: 1;">
                ${navigator.onLine
                    ...
                    : html`<div style="padding: 0px 25px;">${
                        until((async () => unsafeHTML(await run('html', { nwbfile_path: filepath })))(), '<small>Loading HTML representation...</small>')
                    }</div>`}
            </div>
            <div style="padding-left: 20px; display: flex; flex-direction: column;">
                ...
                ${until((async () => {
                    const list = new InspectorList({ items: await run('inspect_nwbfile', { nwbfile_path: filepath, ignore: ["check_description"] }), listStyles: { maxWidth: "350px" } });
