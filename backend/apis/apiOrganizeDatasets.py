...
class BfGetDatasetFilesFolders(Resource):
    ...
    @api.doc(
        ...
        description="Import a dataset from Pennsieve and populate the local SODA JSON object.",
    )
...
class GenerateDatasetLocally(Resource):
    ...
    parser_change_dataset_status.add_argument(
        ...
        type=str,
        required=True,
        help="The final destination to generate the dataset in. Valid option is create new.",
        location="json",
    )
    parser_change_dataset_status.add_argument(
        ...
        type=str,
        required=True,
        help="The local path to generate the dataset.",
        location="json",
    )
    parser_change_dataset_status.add_argument(
        "dataset_name",
        type=str,
        required=True,
        help="The name of the dataset being generated.",
        location="json",
    )
    parser_change_dataset_status.add_argument(
        "soda_json_directory_structure",
        type=dict,
        required=True,
        help="The sodajsonobject dataset directory structure.",
        location="json",
    )
    ...
    @api.doc(
        responses={200: "Success", 400: "Bad Request", 500: "Internal Server Error"},
        description="Generate a dataset at the given local directory using the sodajsonobject dataset directory structure.",
    )
    def post(self):
...
class ImportDataset(Resource):
    ...
    parser_import_dataset.add_argument(
        ...
        type=dict,
        required=True,
        help="The sodajsonobject filled with the bfaccount and dataset info available.",
        location="json",
    )
    parser_import_dataset.add_argument(
        "root_folder_path",
        type=str,
        required=True,
        help="The local path to import the dataset.",
        location="json",
    )
    parser_import_dataset.add_argument(
        "irregular_folders",
        type=list,
        required=True,
        help="The name of the dataset being imported.",
        location="json",
    )
    parser_import_dataset.add_argument(
        "replaced",
...
model_import_dataset_organize_datasets_progress_response = api.model(
    "ImportDatasetOrganizeDatasetsProgressResponse",
    {
        "create_soda_json_progress": fields.Integer(
            required=True,
            description="The progress of the create soda json object operation.",
        ),
        "create_soda_json_total_items": fields.Integer(
            required=True,
            description="The total number of items to be processed in the create soda json object operation.",
        ),
        "progress_percentage": fields.Integer(
            required=True,
            description="The percentage of the create soda json object operation completed.",
        ),
        "create_soda_json_completed": fields.Integer(
            required=True,
            description="Whether the create soda json object operation has completed.",
        ),
    },
)
...
class ImportDatasetOrganizeDatasetsProgress(Resource):
    ...
    @api.doc(
        responses={200: "Success", 500: "Internal Server Error"},
        description="Get the progress of the Organize Datasets import dataset operation.",
    )
    def get(self):
        try:
...
class ImportDatasetPennsieveProgress(Resource):
    @api.doc(
        responses={200: "Success", 500: "Internal Server Error"},
        description="Returns the progress of the current Pennsieve dataset import operation. Used in Organize Datasets.",
    )
    def get(self):
        try:
