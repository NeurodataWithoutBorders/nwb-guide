...
from os.path import isdir, join, exists, expanduser, dirname, getsize
import time
import shutil
...
def bf_add_account_api_key(keyname, key, secret):
    ...
    try:
        ...
        abort(401, "Please check that key name, key, and secret are entered properly")
    ...
    try:
        ...
        if org_id != "N:organization:618e8dd9-f8d2-4dc4-9abb-c6aaab2e78a0":
            abort(403, "Please check that your account is within the SPARC Organization")
...
def bf_submit_dataset(accountname, bfdataset, pathdataset):
    ...
    if not isdir(pathdataset):
        submitdatastatus = "Done"
        error_message = f"{error_message}Please select a valid local dataset folder<br>"
        did_fail = True
        did_upload = False
        c += 1
    ...
    if role not in ["owner", "manager", "editor"]:
        submitdatastatus = "Done"
        error_message = "You don't have permissions for uploading to this Pennsieve dataset"
        did_fail = True
        did_upload = False
        abort(403, error_message)
...
def bf_get_permission(selected_bfaccount, selected_bfdataset):
    ...
    try:
        # user permissions
        selected_dataset_id = myds.id
        list_dataset_permission = bf._api._get(f"/datasets/{str(selected_dataset_id)}/collaborators/users")
        ...
        for i in range(len(list_dataset_permission)):
            ...
            list_dataset_permission_first_last_role.append(f"User: {first_name} {last_name} , role: {role}")
        ...
        if type(list_dataset_permission_organizations) is dict:
            ...
            if "role" in organization_keys:
                ...
                list_dataset_permission_first_last_role.append(
                    "Organization: " + organization_name + ", role: " + organization_role
                )
        else:
        ...
        return {
            "permissions": list_dataset_permission_first_last_role,
            "team_ids": team_ids,
        }
        ...
        try:
            ...
            for i in range(len(list_dataset_permission)):
                ...
                if first_name == first_name_current_user and last_name == last_name_current_user:
                    if role not in ["owner", "manager"]:
                        abort(
                            403,
                            "You must be dataset owner or manager to change its permissions",
                        )
            ...
            if selected_role == "remove current permissions":
                ...
                bf._api.datasets._put(
                    "/" + str(selected_dataset_id) + "/collaborators/owner".format(dataset_id=selected_dataset_id),
                    json={"id": selected_user_id},
                )
...
def bf_add_permission_team(selected_bfaccount, selected_bfdataset, selected_team, selected_role):
    ...
    if selected_team == "SPARC Data Curation Team":
        if bf.context.id != "N:organization:618e8dd9-f8d2-4dc4-9abb-c6aaab2e78a0":
            abort(
                403,
                "Please login under the Pennsieve SPARC Organization to share with the Curation Team",
            )
    if selected_team == "SPARC Embargoed Data Sharing Group":
        if bf.context.id != "N:organization:618e8dd9-f8d2-4dc4-9abb-c6aaab2e78a0":
            abort(
                403,
                "Please login under the Pennsieve SPARC Organization to share with the SPARC consortium group",
            )
...
def bf_add_banner_image(selected_bfaccount, selected_bfdataset, banner_image_path):
    ...
    try:
        ...
        if isdir(image_folder) and ("SODA" in image_folder) and ("guided-banner-images" not in image_folder):
...
def bf_change_dataset_status(selected_bfaccount, selected_bfdataset, selected_status):
    ...
    try:
        ...
        if role not in ["owner", "manager"]:
            abort(
                403,
                "You don't have permissions for changing the status of this Pennsieve dataset",
            )
    except Exception as e:
        raise e
...
def get_pennsieve_api_key_secret(email, password, keyname):
    ...
    try:
        ...
        error = (
            "Could not switch to the SPARC Organization. Please log in and switch to the organization and try again."
        )
        raise error
