        ...
        if name not in interface_info.keys():
            ...
            interface_info[name] = {
                ...
                "category": os.path.basename(
                    os.path.dirname(os.path.dirname(inspect.getfile(interface)))
                ),
            }
...
def get_schema(interface):
    ...
    """
    if not interface:
        interface = interface_info.keys()  # get schema for all interfaces

    # Single Interface
    if isinstance(interface, str):
        return interfaces[interface_info[interface]["name"]].get_source_schema()
    ...
    class CustomNWBConverter(NWBConverter):
        data_interface_classes = dict(
            zip(
                interface,
                map(lambda name: interfaces[interface_info[name]["name"]], interface),
            )
        )
