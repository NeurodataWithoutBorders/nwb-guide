...
def bf_get_doi(selected_bfaccount, selected_bfdataset):
    ...
    if role not in ["owner", "manager"]:
        abort(
            403,
            "You don't have permissions to view/edit DOI for this Pennsieve dataset",
        )
...
def construct_publication_qs(publication_type, embargo_release_date):
    ...
    """
    return (
        f"?publicationType={publication_type}&embargoReleaseDate={embargo_release_date}"
        if embargo_release_date
        else f"?publicationType={publication_type}"
    )
...
def bf_withdraw_review_dataset(selected_bfaccount, selected_bfdataset):
    ...
    try:
        ...
        abort(
            500,
            "An internal server error prevented the request from being fulfilled. Please try again later.",
        )
...
def update_files_excluded_from_publishing(
    ...
    r = requests.put(
        f"{PENNSIEVE_URL}/datasets/{selected_dataset_id}/ignore-files",
        json=files_excluded_from_publishing,
        headers=headers,
    )
