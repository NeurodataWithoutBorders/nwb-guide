...
model_success_message_response = api.model(
...
class BfGetDoi(Resource):
    ...
    @api.doc(
        responses={400: "Validation Error", 500: "Internal Server Error"},
        description="Reserve a DOI for a dataset",
    )
...
model_ignore_files_object = api.model(
    "IgnoreFilesObject",
    {
...
model_ignore_files_response = api.model(
    "IgnoreFilesResponse",
    {
        "ignore_files": fields.List(
            fields.Nested(model_ignore_files_object),
            required=True,
            description="Files excluded from publishing",
        )
    },
)
...
class BfIgnoreFiles(Resource):
    ...
    parser_ignore_files_put.add_argument(
        "ignore_files",
        type=list,
        help="Files excluded from publishing",
        location="json",
        required=True,
    )
    ...
    @api.doc(
        responses={400: "Validation Error", 500: "Internal Server Error"},
        description="Ignore files from publishing",
    )
...
class PublishingStatus(Resource):
    ...
    @api.doc(
        responses={400: "Validation Error", 500: "Internal Server Error"},
        description="Get the publishing status of a dataset.",
    )
    @api.expect(parser)
...
class PublicationRequest(Resource):
    ...
    publication_parser_post.add_argument(
        "publication_type",
        type=str,
        help="Type of publication (string).",
        location="json",
        required=True,
    )
    publication_parser_post.add_argument(
        ...
        type=str,
        help="(optional) Date at which embargo lifts from dataset after publication",
        location="json",
        required=False,
    )
    ...
    def post(self, dataset_name_or_id):
        ...
        try:
            return bf_submit_review_dataset(
                selected_account,
                dataset_name_or_id,
                publication_type,
                embargo_release_date,
            ...
            if type(e).__name__ == "HTTPError":
                api.abort(
                    400,
                    "Ensure the publication type is valid and that the embargo release date is no more than a year out.",
                )
            if notBadRequestException(e):
                api.abort(500, str(e))
            raise e
