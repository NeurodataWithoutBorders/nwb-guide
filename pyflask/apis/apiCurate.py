...
model_check_empty_files_folders_response = api.model(
    "CheckEmptyFilesFoldersResponse",
    {
...
class CheckEmptyFilesFolders(Resource):
    ...
    @api.doc(
        ...
        description="Given a sodajsonobject return a list of empty files and folders should they exist, as well as the sodajsonobject.",
...
class CurationProgress(Resource):
    ...
    @api.doc(
        ...
        description="Return important details to the client about the state of the currently running curation function.",
    )
    def get(self):
        try:
...
model_curation_file_details_response = api.model(
    "CurationFileDetailsResponse",
    {
        "main_curation_uploaded_files": fields.Integer(
            description="Number of files that have been uploaded thus far. "
        ),
...
class CurationFileDetails(Resource):
    ...
    @api.doc(
        responses={500: "There was an internal server error"},
        description="Function frequently called by front end to help keep track of the amount of files that have been successfully uploaded to Pennsieve, and the size of the uploaded files. Also tells us how many files have been copied (double usage of both variables) to a destination folder for local dataset generation.",
    )
    def get(self):
        try:
...
class GenerateManifestFiles(Resource):
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to either a local dataset or a SODA directory for storing temporary manifest files. The latter is used for editing manifest files for local datasets. The former for editing manifest files of manifest information pulled from Pennsieve.",
        location="json",
    )

    @api.doc(
        responses={500: "There was an internal server error", 400: "Bad Request"},
        description="Generate manifest files in a local temporary directory or in the user's dataset directory. Allows users to edit their manifest files in the standalone manifest file generator feature.",
    )
    @api.expect(parser)
...
model_generate_manifest_locally_response = api.model(
    "GenerateManifestLocallyResponse",
    {
        "success_message_or_manifest_destinationa": fields.String(
            description="Success message or path to the manifest file"
        ),
    },
)
...
class GenerateManifestLocally(Resource):
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Can be edit-manifest or otherwise. Use edit-manifest when a user wants to edit their manifest files in the standalone manifest file generation feature.",
        location="json",
    )
    parser.add_argument(
        "soda_json_object",
...
class GenerateManifestData(Resource):
    ...
    parser.add_argument(
        ...
        type=dict,
        required=True,
        help="dataset structure used to generate manifest files for each high level folder",
        location="json",
    )

    @api.doc(
        responses={500: "There was an internal server error", 400: "Bad Request"},
        description="Generate Manifest Data for each of the high level folders. Returns an array of arrays to be inserted into jspreadsheet.",
    )
    @api.expect(parser)
    def post(self):
        data = self.parser.parse_args()
...
model_dataset_size_response = api.model(
    "DatasetSizeResponse",
    {
