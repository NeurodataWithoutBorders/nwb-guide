...
model_save_submission_file_response = api.model(
    "saveSubmissionFileResponse",
    {
...
class SaveSubmissionFile(Resource):
    ...
    parser_save_submission_file.add_argument(
        "upload_boolean",
        type=boolean,
        help="Boolean to indicate whether to upload the file to the Bionimbus server",
        location="json",
        required=True,
    )
    parser_save_submission_file.add_argument(
    ...
    parser_save_submission_file.add_argument(
        "selected_dataset",
        type=str,
        help="Pennsieve dataset name",
        location="args",
        required=True,
    )
    parser_save_submission_file.add_argument(
        "filepath", type=str, help="Path to the file to be uploaded", location="json"
    )
    parser_save_submission_file.add_argument(
        "submission_file_rows",
        type=list,
        help="List of objects that contain award, milestone, and date properties with appropriate values.",
        location="json",
        required=True,
    )
    ...
    @api.doc(
        description="Save a submission file locally or in a dataset stored on the Pennsieve account of the current user.",
        responses={500: "Internal Server Error", 400: "Bad Request", 403: "Forbidden"},
    )
    def post(self):
        ...
        if not upload_boolean and filepath is None:
            api.abort(
                400,
                "Please provide a destination in which to save your Submission file.",
            )
    ...
    parser_get_submission_file.add_argument(
        "filepath",
        type=str,
        help="Path to the submission file on the user's machine",
        location="args",
        required=True,
    )
    ...
    @api.doc(
        description="Get the submission file from the user's machine.",
...
model_upload_RC_file_response = api.model(
...
model_get_RC_file_response = api.model(
    "getRCFileResponse",
    {
...
class RCFile(Resource):
    ...
    parser_get_RC_file.add_argument(
        "file_type",
        type=str,
        help="Either README or CHANGES from the user's Pennsieve account and dataset.",
        location="args",
        required=True,
    )
    parser_get_RC_file.add_argument(
    ...
    @api.doc(
        description="Get the readme or changes file from Pennsieve.",
        responses={500: "Internal Server Error", 400: "Bad Request"},
    )
    def get(self):
    ...
    @api.doc(
        description="Create a readme or changes file on the given dataset for the given Pennsieve account.",
        responses={500: "Internal Server Error", 400: "Bad Request", 403: "Forbidden"},
    )
    def post(self):
...
model_get_ds_description_file_response = api.model(
    "getDSDescriptionFileResponse",
    {
        "Basic information": fields.List(
            fields.List(fields.String, required=True),
            required=True,
            description="Basic information",
        ),
        "Study information": fields.List(
            fields.List(fields.String, required=True),
            required=True,
            description="Study information",
        ),
        "Contributor information": fields.List(
            fields.List(fields.String, required=True),
            required=True,
            description="Contributor information",
        ),
        "Award information": fields.List(
            fields.List(fields.String, required=True),
            required=True,
            description="Award information",
        ),
        "Related information": fields.List(
            fields.List(fields.String, required=True),
            required=True,
...
class DatasetDescriptionFile(Resource):
    ...
    parser_get_ds_description_file.add_argument(
        "filepath",
        type=str,
        help="Path to the dataset description file on the user's machine or Pennsieve.",
        location="args",
        required=True,
    )
    parser_get_ds_description_file.add_argument(
        "import_type",
        type=str,
        help="For this endpoint needs to be local.",
        location="args",
        required=True,
    )
    ...
    @api.doc(
        description="Get the dataset description file from the user's machine. A separate route exists for grabbing from Pennsieve.",
        responses={500: "Internal Server Error", 400: "Bad Request"},
    )
    def get(self):
    ...
    @api.doc(
        description="Save the dataset description file to the user's machine. A separate route exists for saving to Pennsieve.",
        responses={500: "Internal Server Error", 400: "Bad Request", 403: "Forbidden"},
    )
    def post(self):
        ...
        if upload_boolean and not selected_account and not selected_dataset:
            api.abort(
                400,
                "Error:  To save a dataset description file on Pennsieve provide a dataset and pennsieve account.",
            )

        if not upload_boolean and not filepath:
            api.abort(
                400,
                "Error:  To save a dataset description file on the user's machine provide a filepath.",
            )
...
class CodeDescriptionFile(Resource):
    ...
    parser_upload_code_description_file.add_argument(
        "filepath",
        type=str,
        help="Path to the code description file on the user's machine",
        location="json",
        required=True,
    )
    parser_upload_code_description_file.add_argument(
        "selected_account",
        type=str,
        help="Pennsieve account to save the code_description file to",
        location="json",
        required=True,
    )
    parser_upload_code_description_file.add_argument(
        "selected_dataset",
        type=str,
        help="Pennsieve dataset to save the code_description file to",
        location="json",
        required=True,
    )
    ...
    @api.doc(
        description="Upload the code_description file on the user's machine directly to Pennsieve",
        responses={500: "Internal Server Error", 400: "Bad Request"},
    )
    def post(self):
...
class SubjectsFile(Resource):
    ...
    parser_save_subjects_file.add_argument(
        "filepath",
        type=str,
        help="Path to the subjects file on the user's machine.",
        location="json",
        required=False,
    )
    parser_save_subjects_file.add_argument(
        "upload_boolean",
        type=boolean,
        help="Save subjecst on Pennsieve if True else save locally.",
        location="args",
        required=True,
    )
    parser_save_subjects_file.add_argument(
        "selected_account",
        type=str,
        help="Pennsieve account to save the subjects file to.",
        location="json",
        required=False,
    )
    parser_save_subjects_file.add_argument(
        "selected_dataset",
        type=str,
        help="Pennsieve dataset to save the subjects file to.",
        location="json",
        required=False,
    )
    parser_save_subjects_file.add_argument(
    ...
    @api.doc(
        description="Save the subjects file to the user's machine or to Pennsieve.",
        responses={500: "Internal Server Error", 400: "Bad Request", 403: "Forbidden"},
    )
    def post(self):
        ...
        if upload_boolean and not selected_account and not selected_dataset:
            api.abort(
                400,
                "To save a subjects file on Pennsieve provide a dataset and pennsieve account.",
            )
    ...
    parser_create_data_frames.add_argument(
        "type",
        type=str,
        help="Subjects or Samples are the valid types.",
        location="args",
        required=True,
    )
    parser_create_data_frames.add_argument(
        "filepath",
        type=str,
        help="Path to the subjects or samples file on the user's machine.",
        location="args",
        required=True,
    )
    parser_create_data_frames.add_argument(
        "ui_fields",
        type=str,
        help="The fields to include in the final data frame.",
        location="args",
        required=True,
    )
    ...
    @api.doc(
        description="Get a local subjects file data in the form of data frames.",
        responses={500: "Internal Server Error", 400: "Bad Request"},
    )
    def get(self):
...
model_get_samples_result = api.model(
    "GetSamplesResult",
    {
        "sample_file_rows": fields.List(
            fields.List(fields.String, description="A sample file field."),
            description="A row of sample file fields.",
        ),
    },
)
...
class SamplesFile(Resource):
    ...
    parser_save_samples_file.add_argument(
        "filepath",
        type=str,
        help="Path to the samples file on the user's machine.",
        location="json",
        required=False,
    )
    parser_save_samples_file.add_argument(
        "upload_boolean",
        type=boolean,
        help="Save samples on Pennsieve if True else save locally.",
        location="args",
        required=True,
    )
    parser_save_samples_file.add_argument(
        "selected_account",
        type=str,
        help="Pennsieve account to save the samples file to.",
        location="json",
        required=False,
    )
    parser_save_samples_file.add_argument(
        "selected_dataset",
        type=str,
        help="Pennsieve dataset to save the samples file to.",
        location="json",
        required=False,
    )
    parser_save_samples_file.add_argument(
    ...
    @api.doc(
        description="Save the samples file to the user's machine or to Pennsieve.",
        responses={500: "Internal Server Error", 400: "Bad Request", 403: "Forbidden"},
    )
    ...
    def post(self):
        ...
        if upload_boolean and not selected_account and not selected_dataset:
            api.abort(
                400,
                "Error:  To save a samples file on Pennsieve provide a dataset and pennsieve account.",
            )

        if not upload_boolean and not filepath:
            api.abort(
                400,
                "Error:  To save a samples file on the user's machine provide a filepath.",
            )
    ...
    parser_create_data_frames.add_argument(
        "type",
        type=str,
        help="samples.xlsx is the valid metadata type.",
        location="args",
        required=True,
    )
    parser_create_data_frames.add_argument(
    ...
    @api.expect(parser_create_data_frames)
    @api.doc(
        description="Get a local samples file data in the form of data frames.",
        responses={500: "Internal Server Error", 400: "Bad Request"},
    )
    ...
    def get(self):
        ...
        if ui_fields:
            ...
            ui_fields = list(
                map(
                    str.strip,
                    ui_fields.strip("][").replace("'", "").replace('"', "").split(","),
                )
            )
...
model_ui_fields = api.model(
    "UIFields",
    {
        "ui_fields": fields.List(
            fields.String,
            description="For samples and subjects only. Every header from the respective file in list formatting.",
            required=False,
        ),
    },
)
...
class ImportBFMetadataFile(Resource):
    ...
    parser_import_metadata_file.add_argument(
        "selected_account",
        type=str,
        help="Pennsieve account to save the metadata file to.",
        location="args",
        required=True,
    )
    parser_import_metadata_file.add_argument(
        "selected_dataset",
        type=str,
        help="Pennsieve dataset to save the metadata file to.",
        location="args",
        required=True,
    )
    parser_import_metadata_file.add_argument(
        "file_type",
        type=str,
        help="The type of metadata file that we can import from Pennsieve. Must be [subjects.xlsx, samples.xlsx, dataset_description.xlsx, and submission.xlsx]",
        location="args",
        required=True,
    )
    parser_import_metadata_file.add_argument(
        "ui_fields",
        type=str,
        help="Path to the metadata file on the user's machine.",
        location="args",
        required=False,
    )
    ...
    @api.doc(
        description="Import a metadata file from Pennsieve. NOTE: CONTRARY TO THE SWAGGER UI THE PAYLOAD IS ONLY REQUIRED FOR SUBJECTS AND SAMPLES FILES.",
        responses={500: "Internal Server Error", 400: "Bad Request"},
    )
    ...
    def get(self):
        ...
        if not valid:
            api.abort(
                400,
                "Error: To import a metadata file from Pennsieve provide a file_type, selected_account, and selected_dataset.",
            )
        ...
        if file_type not in [
            ...
            api.abort(
                400,
                "Error: The file_type parameter must be submission.xlsx, samples.xlsx, subjects.xlsx, or dataset_description.xlsx.",
            )

        if file_type in ["samples.xlsx", "subjects.xlsx"] and not ui_fields:
            api.abort(
                400,
                "An ui_fields property is required for fetching samples or subjects file types.",
            )
...
class ImportMilestone(Resource):
    ...
    parser_import_milestone.add_argument(
        "path",
        type=str,
        help="Path to the local data deliverables document",
        location="args",
    )

    @api.expect(parser_import_milestone)
    @api.doc(
        description="Import a milestone from the user's machine.",
        responses={500: "Internal Server Error", 400: "Bad Request"},
    )
    def get(self):
...
class DeleteManifestDummyFolders(Resource):
    ...
    @api.doc(
        description="Delete the dummy folders created by the manifest tool.",
        responses={500: "Internal Server Error", 400: "Bad Request", 200: "OK"},
    )
...
class GenerateManifestFilesPennsieve(Resource):
    @api.doc(
        ...
        params={
            "soda_json_object": "SODA dataset structure",
    ...
    def post(self):
        ...
        if not selected_account or not selected_dataset or not soda_json_object:
            api.abort(
                400,
                "Error: To generate manifest files for Pennsieve provide a selected_account, selected_dataset, and soda_json_object.",
            )
...
manifest_creation_progress_model = api.model(
    "ManifestCreationProgress",
    {
        "total_manifest_files": fields.Integer(
            description="Total amount of manifest files that need to be created for the current dataset."
        ),
